...
- 뷰포트가 `481px × 800px` 이상일 때만 긴 화면 배치를 쓴다. 이미지와 본문은
  `18px` 간격의 한 묶음이며, 묶음은 화면 높이의 `46%`를 광학 중심으로 삼는다.
  모든 카드가 같은 이미지 기준선과 `196px` 본문 슬롯을 공유한다.
- 너비 `480px` 이하이거나 높이 `800px` 미만이면 기존 모바일 배치를 그대로 쓴다.
