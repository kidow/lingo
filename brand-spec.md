...
- 하단 여백 **한복판**은 장식이 아니라 **"다음 장이 있다"는 신호**다. 무엇으로도
  채우지 않는다. 모서리는 이 규칙이 아니다 — 신호를 내는 것은 스와이프가 지나는
  가운데 축이다.
- 엄지 사정권(화면 하단)에 **가로로 늘어선** 탭 대상을 두지 않는다. 스와이프
  경로와 겹쳐 오터치가 난다. **단일 FAB는 예외로 하되 모서리에 붙여 그 축을
  비켜 간다** — 하단 탭바를 두고 쓴 문장이라 점 하나에는 전제가 다르다.
  지금 그 예외는 찾기 하나뿐이다 (components/search-fab.tsx).
...
| **SearchFab** | 카드 레일(480px) 오른쪽 아래에 뜬 52px 원. `--surface` 바탕에 `--line` 테두리 — `SayButton`과 같은 옷이다. 짙은 원으로 두면 화면에서 가장 튀는 것이 찾기가 된다. 아이콘은 lucide `search`, 이름은 `aria-label`로 준다. 바깥 층은 `pointer-events-none`이라 스와이프를 먹지 않는다 |
