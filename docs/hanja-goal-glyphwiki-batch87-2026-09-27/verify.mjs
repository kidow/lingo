...
const runtime=hanjaStrokeData({glyph:'秊',strokes:8})
assert(runtime);assert.equal(runtime.verificationSource,'ehanja-crosschecked')
assert.equal(proof.groups.length,8);assert.equal(sourceGroups.length,8)
assert.deepEqual(sourceGroups.flat().sort((a,b)=>a-b),[0,1,2,3,4,5,6,7])
...
const editable=readFileSync(new URL('../../public/hanja-strokes/glyphwiki/79ca.json',import.meta.url))
...
console.log(JSON.stringify({passed:true,sourceVersions,engineChecked,rawGroups:8,emptyGroups:0,strokeGroups:8,curvedStrokes:4,runtimeRegistered:1,privateMediaSaved:false}))
