...
assert.equal(actual.reduce((n,e) => n + e.strokes, 0), 44)
...
assert.equal(next.remainingCountMatched.length, 11)
assert.equal(new Set(next.remainingCountMatched.map(e=>e.glyph)).size,11)
...
console.log(JSON.stringify({passed:true,publicSourceAndPointHashes:true,normalizedPathHashes:3,domesticVisualSteps:44,newRuntimeGlyphs:0,nextCountMatchedGlyphs:11}))
