...
const runtime=hanjaStrokeData({glyph:'慌',strokes:13})
assert(runtime);assert.equal(runtime.verificationSource,'ehanja-crosschecked')
assert.equal(proof.groups.length,13);assert.equal(sourceGroups.length,13)
assert.deepEqual(sourceGroups.flat().sort((a,b)=>a-b),Array.from({length:13},(_,i)=>i))
...
const editable=readFileSync(new URL('../../public/hanja-strokes/glyphwiki/614c.json',import.meta.url))
...
assert.deepEqual(outlines[12].map(s=>s.direction),['down','curve','right','up'])
assert.deepEqual(trace[12][0].args.slice(2,4),trace[12][1].args.slice(0,2))
assert.deepEqual(trace[12][1].args.slice(4,6),trace[12][2].args.slice(0,2))
assert.deepEqual(read('./progressive-review.json').reviewedStrokes,Array.from({length:13},(_,i)=>i+1))
...
console.log(JSON.stringify({passed:true,sourceVersions,engineChecked,rawGroups:13,strokeGroups:13,hookSegments:4,runtimeRegistered:1,privateMediaSaved:false}))
