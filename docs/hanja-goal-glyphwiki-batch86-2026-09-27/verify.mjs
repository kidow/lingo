...
const runtime=hanjaStrokeData({glyph:'枏',strokes:8})
assert(runtime);assert.equal(runtime.verificationSource,'ehanja-crosschecked')
assert.equal(proof.groups.length,10);assert.equal(sourceGroups.length,8)
assert.deepEqual(sourceGroups.flat().sort((a,b)=>a-b),[0,1,2,3,5,6,7,8,9])
...
const editable=readFileSync(new URL('../../public/hanja-strokes/glyphwiki/678f.json',import.meta.url))
...
assert.deepEqual(sourceGroups,[[0],[1],[2],[3],[5],[6,7],[8],[9]])
assert.deepEqual(trace[4],[])
assert.deepEqual(proof.groups[4].polygons,[])
assert.equal(trace[7][1].kind,'cdDrawCurve')
...
console.log(JSON.stringify({passed:true,sourceVersions,engineChecked,rawGroups:10,emptyGroups:1,strokeGroups:8,curvedStrokes:3,runtimeRegistered:1,privateMediaSaved:false}))
