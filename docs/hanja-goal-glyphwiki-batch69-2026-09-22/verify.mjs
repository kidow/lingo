...
const runtime=hanjaStrokeData({glyph:'戺',strokes:7})
...
const editable=readFileSync(new URL('../../public/hanja-strokes/glyphwiki/623a.json',import.meta.url))
...
assert.deepEqual(trace[5][0].args.slice(2,4),trace[6][0].args.slice(0,2))
assert.deepEqual(trace[1][0].args.slice(2,4),trace[1][1].args.slice(0,2))
assert.deepEqual(trace[8][0].args.slice(2,4),trace[8][1].args.slice(0,2))
assert.deepEqual(trace[8][1].args.slice(4,6),trace[8][2].args.slice(0,2))
...
console.log(JSON.stringify({passed:true,sourceVersions,engineChecked,rawGroups:9,strokeGroups:7,lastStrokeSegments:4,runtimeRegistered:1,privateMediaSaved:false}))
