...
const split = json('split-review.json')
const source = originals.entries[0]
const sourcePoints = [...source.paths[8].matchAll(/[ML]([-\d.]+) ([-\d.]+)/g)].map(m => [+m[1], +m[2]])
const fragment = points => points.map((p, i) => (i ? 'L' : 'M') + p.join(' ')).join(' ')
if (source.glyph !== '珽' || source.strokes !== 10 || source.catalogStrokes !== 11
  || split.decision !== 'confirmed-split' || split.sourceStroke !== 9 || split.splitPointIndex !== 3
  || split.originalPath !== source.paths[8]
  ...
  || !isDeepStrictEqual(proposals['珽'].map(p => p.derivedFromStroke), split.targetSourceStrokeMapping))
  throw Error('Dictionary split boundary mismatch')
...
const pins = Object.fromEntries(['split-review.json', 'initial-observations.json', 'originals.json', 'observations.json', 'proposals.json', 'source-checks.json', 'corrections.json']
...
const characters = accepted.map(e => {
  ...
  return {
    ...
    geometryCorrection: 'dictionary-g2-geometry-batch16-' + e.glyph.codePointAt(0).toString(16) + '-v1',
