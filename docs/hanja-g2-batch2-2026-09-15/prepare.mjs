...
if (observations.status !== 'complete' || accepted.length !== 50 || observations.entries.length !== 50)
...
for (const observation of accepted) {
  ...
  const paths = recipe.map((r, i) => {
    ...
    if (!edit || edit.path !== r.path || !r.reason) throw Error('Authored path not reviewed')
    return r.path
  })
  pathsByGlyph.set(original.glyph, paths)
}
const candidate = {
  date: '2026-09-15', coordinateSystem: '100 x 100, y downward; licensed normalized centerlines with reviewed corrections',
...
const review = {
  date: '2026-09-15', verificationSource: 'ehanja-crosschecked',
  ...
  reviewedCharacters: 50, reviewedStrokes: 658, approvedCharacters: 50, approvedStrokes: 658,
...
const verification = {
  ...
  scope: 'Grade 2 batch 2: 50 characters individually checked; 17 corrected forms rechecked. Not exam-body certification.',
}
...
const characters = accepted.map(e => {
  ...
  return {
    glyph: e.glyph, verifiedAt: '2026-09-15', geometrySource: geometry[original.corpus].sha256,
    geometryCorrection: 'dictionary-g2-batch2-' + e.glyph.codePointAt(0).toString(16) + '-v1',
