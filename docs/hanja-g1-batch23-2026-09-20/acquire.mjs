...
import { parseCandidates, MAKE_ME_A_HANZI_SOURCE, CANDIDATE_SOURCE, JAPANESE_CANDIDATE_SOURCE, TRADITIONAL_CANDIDATE_SOURCE, SIMPLIFIED_CANDIDATE_SOURCE } from '../../scripts/hanja-stroke-audit.ts'
const read = file => readFileSync(new URL(file, import.meta.url))
export async function acquire() {
  const batch = JSON.parse(read('./selection.json'))
  const inventory = JSON.parse(read('../hanja-g1-inventory-2026-09-18/dictionary-inventory.json'))
  const dictionary = inventory.rows.map(row => Object.fromEntries(inventory.fields.map((key, i) => [key, row[i]])))
  // 几 has only a Ko (AnimCJK Korean) candidate with a matching stroke count; pin that corpus too.
