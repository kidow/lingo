...
def dictionary_svg(glyph, end, colored=True, key=''):
    ...
    if end > len(strokes):
        # the candidate has more strokes than the dictionary: nothing left to reveal on this side
        grey = ''.join(f'<path d="{html.escape(s["outline"].get("d"))}" fill="#bbb"/>' for s in strokes)
        return '<svg viewBox="0 0 1024 1024"><g transform="' + data['transform'] + '">' + grey + '</g></svg>'
...
def candidate_svg(paths, end, colored=True):
    ...
    if end > len(paths):
        pass  # the dictionary has more strokes than the candidate: nothing left to reveal on this side
    elif colored:
    ...
    def do_GET(self):
        try:
            ...
            if self.path.startswith('/metadata'):
                ...
                total = max(len(paths), len(data['strokes']))
                content = f'<h1>{index+1}/{len(DATA["entries"])} · {glyph} · dictionary {len(data["strokes"])} / candidate {len(paths)} strokes · {entry["corpus"]}</h1>'
                ...
                finish = min(total, int(q.get('end', [str(total)])[0]))
                ...
                content += dictionary_svg(glyph, len(data['strokes']), False, 'final') + candidate_svg(paths, len(paths), False)
                ...
                body = ('<!doctype html><meta charset="utf-8"><style>body{zoom:1;width:800px;margin:10px;background:white;color:#111;font:14px system-ui}h1{font-size:22px;margin:4px 0}p{margin:8px 0}.grid{display:grid;grid-template-columns:repeat(3,262px)}section{border:1px solid #ddd;padding:4px}strong{font-size:13px}.pair{display:flex}.pair svg{width:120px;height:128px}.final{display:flex;justify-content:center}.final svg{width:210px;height:210px}</style>' + content).encode()
...
if __name__ == '__main__':
    server = ThreadingHTTPServer(('127.0.0.1', 51799), Handler)
    print('Special grade II one-stroke-gap review http://127.0.0.1:51799', flush=True)
