...
745(59.6%)에서 끝난다. 56.2%는 바닥이 아니라 **천장에서 네 뼘 아래**다.
...
**여덟은 넣기 전에 뜻을 다시 본다.** 표 아래쪽 여덟(`gymnasium` · `diner` ·
`residence` · `attire` · `accessory` · `periodical` · `vend` · `sedan`)은 뜻이
미세하게 갈린다 — 체육관과 헬스장, 간이식당과 식당, 세단과 자동차는 상하위
관계지 같은 말이 아니다. 곁말은 "또는"으로 붙어 같은 것이라고 가르치므로,
갈리는 것을 넣으면 틀린 것을 가르친다. 이 여덟을 빼면 39개다.
...
- **"547개 남음"은 일감이 아니다.** 일감은 곁말 47(확실 39) + 새 개념 7(확실 3),
  합쳐 쉰 남짓이다.
