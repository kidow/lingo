...
const runtime=hanjaStrokeData({glyph:'坰',strokes:8})
...
assert.deepEqual(sourceGroups.flat().sort((a,b)=>a-b),Array.from({length:10},(_,i)=>i))
...
const editable=readFileSync(new URL('../../public/hanja-strokes/glyphwiki/5770.json',import.meta.url))
...
assert.deepEqual(trace[5][0].args.slice(2,4),trace[5][1].args.slice(0,2))
...
console.log(JSON.stringify({passed:true,sourceVersions,engineChecked,rawGroups:10,strokeGroups:8,enclosureSegments:4,runtimeRegistered:1,privateMediaSaved:false}))
