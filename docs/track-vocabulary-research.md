...
**HSK 재등급을 공식 PDF와 전수 대조했다 (2026-09-06).** 남의 저장소 태그 하나로
1,734장의 등급을 갈았으므로 원본에 대고 확인했다. 표본이 아니라 전수다.

공식 PDF는 암호화돼 있고(`/Encrypt`) 이 기계에는 pdftotext·mutool·qpdf가 없다.
설치하는 대신 macOS가 가진 PDFKit을 썼다 — 열두 줄짜리 Swift로 406쪽을 텍스트로
뽑는다. 표는 `序号 等级 词语 拼音 词性` 꼴이라 줄마다 정규식 하나로 갈린다.
주의할 것 셋: `汉考国际` 워터마크가 줄 가운데로 끼어들고, 등급 칸이 `1（2）（4）`나
`2（7-9）`처럼 겹치며(첫 숫자가 그 낱말을 처음 만나는 등급이다), `别1`·`生2`처럼
동형어 번호가 표기에 붙는다.

11,000행을 하나도 놓치지 않고 읽었다(실패 0). 고유 낱말 10,896개다.

| 대조 | 결과 |
| --- | ---: |
| 우리 콘텐츠에서 등급이 붙은 중국어 낱말 | 1,734 |
| **공식 표와 등급이 같은 것** | **1,734 (100%)** |
| 어긋난 것 | 0 |

저장소 데이터 전체와도 맞춰 봤다. `t` 태그 10,057개는 **공식 표의 부분집합**이다 —
등급이 어긋나는 낱말이 하나도 없고, 공식에만 있는 낱말이 839개다. 즉 이 데이터는
틀린 등급을 주지 않고 **빠뜨리기만 한다.**

빠진 839개 중 51개가 우리 콘텐츠에 있다 — `头痛`(4) · `电子书`(3) · `橙子`(5) ·
`肺炎`(6) · `天花板`(7) 같은 것들이라 등급이 빌 이유가 없는 낱말이다. 공식 표를
직접 쓰면 메워진다. 다만 PDF 추출이 macOS PDFKit에 기대므로, 채택한다면 뽑은 표를
생성물로 커밋하는 쪽이 맞다(`lib/audio-have.ts`와 같은 꼴).
