...
const runtime=hanjaStrokeData({glyph:'阱',strokes:7})
...
const editable=readFileSync(new URL('../../public/hanja-strokes/glyphwiki/9631.json',import.meta.url))
...
assert.deepEqual(sourceGroups,[[0,1],[2],[3],[4],[5],[6],[7]])
assert.equal(trace[1][0].kind,'cdDrawCurve')
assert.equal(trace[2][1].kind,'cdDrawCurve')
assert.equal(trace[6][1].kind,'cdDrawCurve')
