#!/usr/bin/env python3
"""Compare 蔓 source strokes without saving dictionary graphics."""
import sys
...
r.ENTRIES = [{'glyph':'蔓','id':'glyphwiki-u8513-k-width3','dictionaryStrokes':15,
    'dictionary':meta['dictionary'],'viewBox':'0 0 100 100','strokeWidth':3,
    'strokes':[{'path':p} for p in approved['paths']]}]
r.base.renderer.ENTRIES['蔓'] = {'strokes':15,'dictionary':meta['dictionary']}
original_candidate = r.candidate
def candidate(entry, end=None):
    return original_candidate(entry,end).replace('stroke-width="5"','stroke-width="3"').replace('stroke-width="4"','stroke-width="3"')
...
if __name__ == '__main__':
    port = int(sys.argv[1]) if len(sys.argv)>1 else 51891
