...
export function boxPaths(source,groups,order){
  const raw=expandKage(source,{allowConnectionLines:true,allowBoxLines:true})
  assert(raw.every(p=>p.type===1),'Only reviewed straight box lines are admitted')
  ...
  const paths=groups.map(group=>{
    ...
    if(b){
      ...
      assert((a.head===0&&a.tail===0)||(a.head===12&&a.tail===13)
        ||(a.head===2&&a.tail===2),'Unreviewed standalone box line')
