...
export function prepare() {
  ...
  for (const observation of json('./observations.json').filter(o => o.decision === 'matched')) {
    ...
    const recipe = proposals[item.glyph] ? {
      glyph: item.glyph, id: `vivasam-g3-batch5-followup3-${item.glyph.codePointAt(0).toString(16)}-v1`,
    ...
    const record = {
      ...
      notes: observation.notes + ' Evidence: docs/hanja-g3-batch5-followup3-2026-09-15/. Times are sampled completed-by observations, not exact pen lifts. Only this whole-glyph comparison authorizes this entry.',
    }
