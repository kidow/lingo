...
assert.equal(hanjaStrokeData({glyph:'蔭',strokes:15}),null)
assert.equal(proof.groups.length,18)
assert.deepEqual(trace[12][0].args.slice(2,4),[142.57,127.6])
assert.equal(trace[13][0].args[1],132.92000000000002)
assert.notDeepEqual(trace[15][0].args.slice(4,6),trace[16][0].args.slice(0,2))
const alternatives=read('./alternatives.json')
assert.equal(alternatives.records['u852d'].data,'99:0:0:0:0:200:200:u852d-j')
assert.equal(alternatives.records['u4f8c-var-001'].data.split('$')[4],'1:22:32:138:86:117:117')
assert.deepEqual(alternatives.records['u4f8c-var-001'].data.split('$').slice(7),source.records['u4f8c-j'].data.split('$').slice(7))
...
console.log(JSON.stringify({passed:true,sourceVersions,engineChecked,rawGroups:18,domesticStrokes:15,runtimeRegistered:0,held:true,privateMediaSaved:false}))
