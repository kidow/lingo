...
def candidate_svg(paths, end, colored=True):
    ...
    return '<svg viewBox="0 0 100 100"><g fill="none" stroke-width="5" stroke-linecap="round" stroke-linejoin="round">' + ''.join(pieces) + '</g></svg>'
    ...
    def do_GET(self):
        try:
            ...
            if self.path.startswith('/metadata'):
                ...
                detail = '<style>body{zoom:2!important;width:1100px!important}.grid{grid-template-columns:repeat(2,550px)!important}.pair svg{width:265px!important;height:280px!important}.final svg{width:350px!important;height:350px!important}</style>' if q.get('detail', ['0'])[0] == '1' else ''
                content = detail + f'<h1>{index+1}/{len(ENTRIES)} · {glyph} · {len(paths)} strokes · {entry["corpus"]}</h1>'
...
if __name__ == '__main__':
    ...
    print('G2 review http://127.0.0.1:51746', flush=True)
