...
export async function acquire() {
  const batch = JSON.parse(read('../hanja-g2-batch9-2026-09-16/next-batch.json'))
...
if (process.argv[1] && fileURLToPath(import.meta.url) === process.argv[1]) {
  ...
  if (process.argv.includes('--serve')) {
    ...
    createServer((req, res) => {
      ...
      res.end(JSON.stringify({ ...metadata, entries: entries.slice(start, start + Number(u.searchParams.get('count') ?? 1)) }))
    }).listen(51745, '127.0.0.1', () => console.log('Licensed candidate collector ready: ' + data.characters + ' entries in RAM'))
