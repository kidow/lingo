...
export async function audit() {
  const catalogFile = 'content/hanja/characters/g1.json', catalogBytes = read(catalogFile)
  const catalog = JSON.parse(catalogBytes), chars = catalog.characters
  assert.equal(chars.length, 1145)
  assert.equal(new Set(chars.map(c => c.glyph)).size, 1145)
  assert.ok(chars.every(c => c.readingGrade === '1급'))

  // 교과서 목록은 고등 한문 1,800자다. 2급은 한 자도 겹치지 않았으나 1급 겹침은 audit 결과로 확인한다 —
