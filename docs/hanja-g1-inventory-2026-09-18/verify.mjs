...
const catalogBytes = read('../../content/hanja/characters/g1.json'), catalog = JSON.parse(catalogBytes)
const expectedGlyphs = catalog.characters.map(c => c.glyph)
assert.equal(expectedGlyphs.length, 1145)
assert.equal(new Set(expectedGlyphs).size, 1145)
assert.equal(catalog.characters.reduce((n, c) => n + c.strokes, 0), 14898)
assert.ok(catalog.characters.every(c => c.readingGrade === '1급'))

const records = snapshot => {
  assert.equal(snapshot.catalog.sha256, hash(catalogBytes))
  assert.equal(snapshot.catalog.characters, 1145)
  assert.equal(snapshot.catalog.strokes, 14898)
  assert.equal(snapshot.rows.length, 1145)
...
assert.deepEqual(inventory.runtime, { allCharacters: expected.runtimeStrokes, gradeEnabled: expected.gradeEnabled, gradeRemaining: 1145 - expected.gradeEnabled, added: 0 })
if (process.argv.includes('--runtime-baseline')) {
  assert.equal(HANJA_STROKES.length, expected.runtimeStrokes)
  assert.equal(catalog.characters.filter(c => hanjaStrokeData(c)).length, expected.gradeEnabled)
  assert.deepEqual(progress, generated.progress)
}
console.log(JSON.stringify({ status: 'passed', records: 1145, strokes: 14898, dictionary: dictionary.counts,
