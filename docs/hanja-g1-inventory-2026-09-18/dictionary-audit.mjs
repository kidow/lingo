...
export async function auditDictionary({ retryInventory = null } = {}) {
  const catalogPath = 'content/hanja/characters/g1.json'
  const bytes = readFileSync(root + catalogPath)
  const catalog = JSON.parse(bytes)
  assert.equal(catalog.characters.length, 1145)
  ...
  return {
    ...
    catalog: { file: catalogPath, sha256: hash(bytes), characters: catalog.characters.length, strokes: 14898 },
