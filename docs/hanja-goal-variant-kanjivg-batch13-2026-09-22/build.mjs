...
console.log(JSON.stringify(entries.map(review => {
  ...
  if (review.rendering.sourceSha256 !== entry.candidate.sha256 || review.rendering.normalizedStrokeWidth !== ({ "禦": 400 / 109 })[entry.glyph]) throw new Error("Unreviewed rendering width")
  if (entry.catalogStrokes !== 16 || entry.dictionaryStrokes !== 17 || JSON.stringify(review.variant) !== JSON.stringify({ catalogStrokes: 16, playbackStrokes: 17, form: "사전" })) throw new Error("Unreviewed playback variant")
  const paths = review.sourceStrokeIndices.map(i => normalizeKanjiVGPath(entry.paths[i - 1]))
  return {
    glyph: entry.glyph, variant: review.variant, candidateSha256: entry.candidate.sha256, verificationSource: "ehanja-crosschecked", verifiedAt: "2026-09-22",
