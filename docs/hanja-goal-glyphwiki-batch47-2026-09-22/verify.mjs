...
const runtime=hanjaStrokeData({glyph:'菽',strokes:12})
assert(runtime);assert.equal(runtime.verificationSource,'ehanja-crosschecked')
assert.equal(proof.groups.length,13);assert.equal(sourceGroups.length,12)
...
const editable=readFileSync(new URL('../../public/hanja-strokes/glyphwiki/83fd.json',import.meta.url))
...
console.log(JSON.stringify({passed:true,sourceVersions,engineChecked,rawGroups:13,strokeGroups:12,hookSegments:3,runtimeRegistered:1,privateMediaSaved:false}))
