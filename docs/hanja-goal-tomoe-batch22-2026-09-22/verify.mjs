...
assert.equal(actual.reduce((n,e) => n + e.strokes, 0), 54)
...
assert.equal(next.remainingCountMatched.length, 8)
assert.equal(new Set(next.remainingCountMatched.map(e=>e.glyph)).size,8)
...
console.log(JSON.stringify({passed:true,publicSourceAndPointHashes:true,normalizedPathHashes:3,domesticVisualSteps:54,newRuntimeGlyphs:0,nextCountMatchedGlyphs:8}))
