...
const runtime=hanjaStrokeData({glyph:'杇',strokes:7})
assert(runtime);assert.equal(runtime.verificationSource,'ehanja-crosschecked')
assert.equal(proof.groups.length,9);assert.equal(sourceGroups.length,7)
assert.deepEqual(sourceGroups.flat().sort((a,b)=>a-b),Array.from({length:9},(_,i)=>i))
...
const editable=readFileSync(new URL('../../public/hanja-strokes/glyphwiki/6747.json',import.meta.url))
...
assert.deepEqual(trace[6][0].args.slice(2,4),trace[7][0].args.slice(0,2))
assert.deepEqual(trace[7][0].args.slice(2,4),trace[8][0].args.slice(0,2))
assert.deepEqual(trace[8][0].args.slice(4,6),trace[8][1].args.slice(0,2))
assert.deepEqual(asset.enginePreset,{constructorSize:1,style:'mincho',customParameterEdits:false})
const presets=read('./presets.json');assert.equal(presets.length,4);assert.deepEqual(presets.find(p=>p.name==='small-mincho').proof,proof);assert.deepEqual(presets.find(p=>p.name==='small-mincho').trace,trace)
...
console.log(JSON.stringify({passed:true,sourceVersions,engineChecked,rawGroups:9,strokeGroups:7,curvedStrokes:4,runtimeRegistered:1,privateMediaSaved:false}))
