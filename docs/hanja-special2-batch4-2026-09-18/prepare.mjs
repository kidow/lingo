...
if (observations.status !== 'complete' || observations.entries.length !== 50
  || accepted.length + held.length !== 50 || accepted.length !== 48
...
const verification = {
  ...
  scope: `Special grade II batch 4: 50 characters individually checked, ${accepted.length} approved and ${held.length} held; ${corrected} corrected forms rechecked. Not exam-body certification.`,
}
...
const characters = accepted.map(e => {
  ...
  return {
    ...
    geometryCorrection: 'dictionary-special2-batch4-' + e.glyph.codePointAt(0).toString(16) + '-v1',
