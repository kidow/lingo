...
import { validateG2FollowupDictionaryBundle } from '../../scripts/hanja-stroke-dictionary-g2-followup.ts'
...
assert.deepEqual(read('../../public/hanja-strokes/dictionary-reviewed-g2-followup.json'), compiled.runtime)
...
const progress = {
  date: '2026-09-15', batch: { reviewed: 6, reviewedStrokes: 75, applied: 6, appliedStrokes: 75,
    corrected: 6, locallyCorrectedPaths: 18, retainedPaths: 57, held: 0 },
  overall: summarize(characters),
  byGrade: grades.map(grade => ({ grade, ...summarize(characters.filter(c => c.readingGrade === grade)) })),
  next: { task: 'Review the next 50 eligible grade 2 characters', first: entries[0].glyph,
...
console.log(JSON.stringify({ verification: 'passed', compilerMatchesArtifacts: true, proofPinsValid: true,
  uniqueRuntimeGlyphs: true, followupApprovalsComplete: true, progress, nextBatch }, null, 2))
