...
import { validateG2Batch9DictionaryBundle } from '../../scripts/hanja-stroke-dictionary-g2-batch9.ts'
...
assert.deepEqual(read('../../public/hanja-strokes/dictionary-reviewed-g2-batch9.json'), compiled.runtime)
...
const nextBatch = { ...planned.nextBatch, title: '2급 다음 전체 획 검토 후보',
  characters: entries.length, strokes: entries.reduce((n, e) => n + e.strokes, 0), entries }
const progress = {
  date: '2026-09-16', batch: { reviewed: 50, reviewedStrokes: 578, applied: 50, appliedStrokes: 578,
    corrected: 41, locallyCorrectedPaths: 108, retainedPaths: 470, held: 0 },
  overall: summarize(characters),
  byGrade: grades.map(grade => ({ grade, ...summarize(characters.filter(c => c.readingGrade === grade)) })),
  next: { task: 'Review the remaining eligible grade 2 characters', first: entries[0].glyph,
...
console.log(JSON.stringify({ verification: 'passed', compilerMatchesArtifacts: true, proofPinsValid: true,
  uniqueRuntimeGlyphs: true, batch9ApprovalsComplete: true, progress, nextBatch }, null, 2))
