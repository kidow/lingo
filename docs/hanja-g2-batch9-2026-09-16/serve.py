    ...
    def do_GET(self):
        try:
            ...
            if self.path.startswith('/metadata'):
                ...
                detail = '<style>body{zoom:1!important;width:1100px!important}.grid{grid-template-columns:repeat(2,550px)!important}.pair svg{width:265px!important;height:280px!important}.final svg{width:350px!important;height:350px!important}</style>' if q.get('detail', ['0'])[0] == '1' else ''
                content = detail + f'<h1>{index+1}/50 · {glyph} · {len(paths)} strokes · {entry["corpus"]}</h1>'
