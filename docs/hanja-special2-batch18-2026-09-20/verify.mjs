...
import { validateSpecial2Batch18DictionaryBundle } from '../../scripts/hanja-stroke-dictionary-special2-batch18.ts'
import { plan } from '../hanja-special2-inventory-2026-09-18/plan.mjs'
...
assert.deepEqual(read('../../public/hanja-strokes/dictionary-reviewed-special2-batch18.json'), compiled.runtime)
...
const inventory = read('../hanja-special2-inventory-2026-09-18/inventory.json')
const dictionary = read('../hanja-special2-inventory-2026-09-18/dictionary-inventory.json')
...
const heldElsewhere = new Set(readdirSync(docsDir)
  .filter(name => /^hanja-special2-batch\d+-/.test(name))
  .flatMap(name => {
...
// The 2026-09-18 plan only knew the Korean, Make Me a Hanzi and Japanese corpora. The two extra AnimCJK
// files of the same pinned revision are enumerated separately in queue.json; this list stays plan-based.
...
const nextBatch = { ...planned.nextBatch, title: '특급II 다음 전체 획 검토 후보',
  characters: entries.length, strokes: entries.reduce((n, e) => n + e.strokes, 0), entries }
const progress = {
  date: '2026-09-20', batch: { reviewed: 12, reviewedStrokes: compiled.review.reviewedStrokes,
  ...
  // The eligible special grade II queue is empty after this batch: report that plainly instead of a next range.
  next: { task: entries.length ? 'Review the next 50 eligible special grade II characters' : 'No eligible special grade II characters remain; the rest lack a licensed candidate with a matching stroke count or are held',
    first: entries[0]?.glyph ?? null, last: entries.at(-1)?.glyph ?? null, characters: entries.length, strokes: nextBatch.strokes,
...
console.log(JSON.stringify({ verification: 'passed', compilerMatchesArtifacts: true, proofPinsValid: true,
  uniqueRuntimeGlyphs: true, batch18ApprovalsComplete: true, progress, nextBatch }, null, 2))
