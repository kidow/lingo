...
if (observations.status !== 'complete' || observations.entries.length !== 12
  || accepted.length + held.length !== 12 || accepted.length !== 12
...
const review = {
  ...
  reviewedCharacters: 11, reviewedStrokes: originals.entries.reduce((n, e) => n + e.strokes, 0),
...
const verification = {
  ...
  scope: `Special grade II batch 17: 11 characters individually checked, ${accepted.length} approved and ${held.length} held; ${corrected} corrected forms rechecked. Not exam-body certification.`,
}
...
const characters = accepted.map(e => {
  ...
  return {
    ...
    geometryCorrection: 'dictionary-special2-batch18-' + e.glyph.codePointAt(0).toString(16) + '-v1',
