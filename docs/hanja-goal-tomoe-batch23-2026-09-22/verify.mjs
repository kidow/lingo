...
assert.equal(actual.reduce((n,e) => n + e.strokes, 0), 59)
for (const m of metadata) {
  ...
  assert.equal(f.decision, m.glyph === '饐' ? 'approved' : 'hold')
  assert(f.observations.length)
  assert.equal(HANJA_STROKES.some(e => e.glyph === m.glyph), m.glyph === '饐')
}
assert.equal(next.remainingCountMatched.length, 5)
assert.equal(new Set(next.remainingCountMatched.map(e=>e.glyph)).size,5)
...
console.log(JSON.stringify({passed:true,publicSourceAndPointHashes:true,normalizedPathHashes:3,domesticVisualSteps:59,newRuntimeGlyphs:1,nextCountMatchedGlyphs:5}))
