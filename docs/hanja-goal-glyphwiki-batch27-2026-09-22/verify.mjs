...
const expected = read('public/hanja-strokes/dictionary-reviewed-glyphwiki-batch27.json')
...
assert.deepEqual(hanjaStrokeData({ glyph: '芙', strokes: 8 })?.paths, built[0].paths)
assert.equal(hanjaStrokeData({ glyph: '芙', strokes: 7 }), null)
let versions = 0
if (process.argv.includes('--sources')) {
  const source = read('public/hanja-strokes/glyphwiki/8299.json')
...
console.log(JSON.stringify({ passed: true, approvedGlyphs: 1, strokes: 8, sourceVersions: versions, privateGraphicsSaved: false }))
