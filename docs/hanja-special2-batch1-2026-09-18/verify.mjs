...
import { validateSpecial2Batch1DictionaryBundle } from '../../scripts/hanja-stroke-dictionary-special2-batch1.ts'
...
assert.deepEqual(read('../../public/hanja-strokes/dictionary-reviewed-special2-batch1.json'), compiled.runtime)
...
const pending = planned.summary.actionQueue.wholeStrokeReview.glyphs.filter(g => !hanjaStrokeData(characters.find(c => c.glyph === g)))
...
const progress = {
  date: '2026-09-18', batch: { reviewed: 50, reviewedStrokes: compiled.review.reviewedStrokes, applied: 50, appliedStrokes: compiled.review.approvedStrokes,
    corrected: corrected.length, reorderedCharacters: reordered, locallyCorrectedPaths: authored,
    retainedPaths: compiled.review.approvedStrokes - authored, held: 0 },
  overall: summarize(characters),
  byGrade: grades.map(grade => ({ grade, ...summarize(characters.filter(c => c.readingGrade === grade)) })),
  next: { task: 'Review the next 50 eligible special grade II characters', first: entries[0].glyph,
    last: entries.at(-1).glyph, characters: entries.length, strokes: nextBatch.strokes,
    eligibleUnreviewedCharacters: pending.length },
}
console.log(JSON.stringify({ verification: 'passed', compilerMatchesArtifacts: true, proofPinsValid: true,
  uniqueRuntimeGlyphs: true, batch1ApprovalsComplete: true, progress, nextBatch }, null, 2))
