...
export async function audit() {
  const catalogFile = 'content/hanja/characters/g2.json', catalogBytes = read(catalogFile)
  const catalog = JSON.parse(catalogBytes), chars = catalog.characters
  assert.equal(chars.length, 538)
  assert.equal(new Set(chars.map(c => c.glyph)).size, 538)
  assert.ok(chars.every(c => c.readingGrade === '2급'))
  ...
  const groups = { wholeStrokeReview: [], sourceMissing: [], sourceFormReview: [], geometryCountReview: [], geometryNeeded: [], sourceUnavailable: [] }
  for (const r of rows) {
    const [glyph, strokes, , , match, mm, ja, ko, status, bytes, , , isMp4] = r
    if (match === 'absent') groups.sourceMissing.push(glyph)
    else if (match !== 'exact') groups.sourceFormReview.push(glyph)
  ...
  for (const row of rows) {
    ...
    const group = counts.every(count => count === null) ? 'missing'
      : counts.includes(row[1]) ? 'countMatch' : 'countReview'
  ...
  return {
    version: 1, date: '2026-09-15', purpose: 'Source and candidate-count discovery only; no per-stroke visual review or runtime approval.',
    ...
    runtime: { allCharacters: HANJA_STROKES.length, grade2Enabled: runtimeCount, grade2Remaining: chars.length - runtimeCount, added: 0 },
    ...
    rows, publisherGroups: groups, geometryGroups,
    publisherFirst50: { characters: first50.length, strokes: first50.reduce((n, c) => n + c.strokes, 0), entries: first50, status: 'awaiting-complete-order-direction-boundary-and-form-review' },
