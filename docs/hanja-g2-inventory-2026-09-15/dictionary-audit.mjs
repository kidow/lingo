...
export async function auditDictionary({ retryInventory = null } = {}) {
  const catalogPath = 'content/hanja/characters/g2.json'
  const bytes = readFileSync(root + catalogPath)
  const catalog = JSON.parse(bytes)
  assert.equal(catalog.characters.length, 538)
  ...
  return {
    schemaVersion: 1, checkedAt: '2026-09-15', purpose: 'Public dictionary availability and metadata inventory only; no visual stroke approval.',
    catalog: { file: catalogPath, sha256: hash(bytes), characters: catalog.characters.length, strokes: 6668 },
