...
export function plan() {
  ...
  return {
    summary: {
      schemaVersion: 1, checkedAt: '2026-09-15', scope: '2급 신규 배정 538자; metadata inventory, not stroke approval.',
      inputs: ['inventory.json', 'dictionary-inventory.json'].map(file => ({ file, sha256: hash(read('./' + file)) })),
      catalog: inventory.catalog, publisher: { exactMatches: candidates.filter(c => c.publisherMatch === 'exact').length, missing: inventory.publisherGroups.sourceMissing.length },
    ...
    nextBatch: {
      schemaVersion: 1, title: '2급 첫 전체 획 검토 후보', basis: 'Exact dictionary title, consistent source metadata and at least one licensed candidate with matching catalog stroke count. These are intake conditions, not approval.',
    ...
    progress: { checkedAt: '2026-09-15', total, applied: HANJA_STROKES.length, remaining: total - HANJA_STROKES.length,
