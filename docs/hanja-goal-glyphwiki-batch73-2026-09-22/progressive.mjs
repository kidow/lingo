...
export const sourceGroups = [[0],[1],[3],[2],[4],[5,6]]
const CURVES = {
  "4:1": [
    88,
    96.225,
...
export function compileProgressive(trace, proof) {
  ...
  return sourceGroups.map(indices=>indices.flatMap(index=>{
    ...
    return trace[index].flatMap((call,j)=>{
      ...
      if(index===6&&j===2){
        assert.deepEqual(a,[144,178.875,181,178.875,6,5])
        assert.equal(call.polygons.length,3)
        assert.deepEqual(call.polygons[2].map(p=>[p.x,p.y]),[[181,172.8],[183,147.8],[181,147.8],[175,172.8]])
        return [segment(call.polygons.slice(0,2),'right',(x2-x1)/2),segment(call.polygons.slice(2),'up',(172.8-147.8)/2)]
      }
