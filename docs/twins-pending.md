...
## 한글로 가르는 무리 셋 — 다 그리면 이백쉰 쌍이 된다 (2026-09-21)

나라 이름·주의자·달 이름처럼 **그릴 것이 없는 낱말**은 임자가 한글을 그림에
넣는 길을 골랐다(「글자를 뜻으로 삼는 낱말」, [IMAGE_STYLE](../IMAGE_STYLE.md)).
같은 판에 색과 한글만 다르므로 `pnpm twins`는 이것을 죄다 짚는다.

| 무리 | 수 | 생김새 | 그려진 것 |
| --- | ---: | --- | ---: |
| 달력 | 12 | 머리띠에 「8월」 · 빈 날짜 칸 | 12 |
| 기 | 16 | 짧은 장대의 세모꼴 기에 「모스크바」 | 2 |
| 이름패 | 12 | 선 나무패에 「공산주의자」 | 0 |

**셋 다 흠이 아니다** — 갈래는 한글이 낸다. 다시 훑을 때 건너뛴다.

**다만 무리마다 붙는 정도가 다르다.** 재 보니 이렇다.

    of-moscow          ↔ of-the-soviets    구조  27 · 색 0.241   문턱 안
    calendar-august    ↔ calendar-september 구조 31 · 색 0.15    문턱 안
    a-communist-person ↔ a-patriot-person  구조  93 · 색 0.358   멀다

**한글이 그림에서 차지하는 넓이가 가른다.** 이름패는 패가 화면을 거의 다
채우고 한글이 그 안에 크게 들어가, 「공산주의자」와 「애국자」의 획 생김새가
구조에 그대로 실린다. 기와 달력은 한글이 작고 나머지가 같은 판이라 붙는다.

그러니 다 그려도 twins가 짚는 것은 **기 백스무 쌍과 달력 예순여섯 쌍**이고,
이름패 열둘은 아마 안 걸린다. 지금은 열네 장만 그려져 두 쌍만 나온다.

색으로 가르려는 시도는 힘을 못 쓴다 — 자주와 잿빛이 0.241이고
`calendar-april`과 `calendar-may`는 0.08이다. 갈라야 한다면 색이 아니라
**꼴**을 바꾼다.

### 한글이 든 기 열여섯 — 흠이 아니지만 다 그리면 백스무 쌍이 된다 (2026-09-21)

`of-moscow`(모스크바의)와 `of-the-soviets`(소비에트의)를 뽑았더니 **구조 27 ·
색 0.241**이 나왔다. 문턱(50 · 0.20) 안쪽이다.

둘 다 **짧은 장대에 걸린 세모꼴 기**이고 머리띠 색과 한글만 다르다. 세어 보니
같은 꼴이 **열여섯**이다 — city에 둘, travel에 열넷이다.

    of-moscow · of-the-soviets · of-russia · of-england · of-spain · of-germany ·
    of-france · of-china · of-america · of-siberia · of-the-slavs ·
    of-africa-the-land · a-man-of-africa · a-man-of-europe · a-man-of-russia ·
    a-man-of-siberia

**흠이 아니다.** 나라·고장 이름은 그릴 것이 없어 임자가 한글을 그림에 넣는
길을 골랐고(「글자를 뜻으로 삼는 낱말」, IMAGE_STYLE), 갈래는 그 한글이 낸다.
달력 무리와 같은 판정이다.

**다만 지금 그려진 것이 둘뿐이다.** 열넷을 마저 그리면 `pnpm twins`가
열여섯 가운데 두 개씩 짝지어 **백스무 쌍**을 짚는다. 다시 훑을 때 이 무리는
통째로 건너뛴다.

색으로 가르려는 시도는 거의 힘을 못 쓴다 — 자주와 잿빛인데 색 거리가
0.241이다. 더 갈라야 한다면 색이 아니라 **기의 꼴**(제비꼬리 · 네모 · 세모)이나
장대의 생김새를 바꾸는 편이 낫다. 임자가 정할 일이라 여기 적어만 둔다.
