...
- `clock`↔`quarter-to` 59 — **둘 다 시계인 것이 옳다.** 십오 분 전은 시계가
  가리키는 시각이라 얼굴을 버릴 수 없다. 그림이 아니라 뜻이 이웃한 자리다.
- `quarter-to`↔`tide-clock` 53 — 조석 시계도 시계라 얼굴을 버릴 수 없다.
  갈래를 내려면 **테와 눈금**을 바꾼다 — 반원 판, 또는 나무 판에 박힌 굵은 테.
- `clock`↔`tide-clock`은 문턱 밖이다.
...
| `sport/cycling`(사이클)<br>`sport/cyclist`(사이클 선수) | 50 · 0.25 | 둘 다 **경주 자전거에 탄 인물 하나**를 옆에서 본다. 활동과 사람을 같은 그림으로 그렸다 | **`cycling`을 옮긴다** — 사람 하나가 아니라 **여럿이 줄지어 달리는 무리**로. 활동은 무리가, 사람은 혼자가 보여준다 |
