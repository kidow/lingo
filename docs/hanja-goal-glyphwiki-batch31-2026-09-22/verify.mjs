...
const expected = read('public/hanja-strokes/dictionary-reviewed-glyphwiki-batch31.json')
...
assert.deepEqual(hanjaStrokeData({ glyph: '葵', strokes: 13 })?.paths, built[0].paths)
assert.equal(hanjaStrokeData({ glyph: '葵', strokes: 14 }), null)
let versions = 0
if (process.argv.includes('--sources')) {
  const source = read('public/hanja-strokes/glyphwiki/8475.json')
...
console.log(JSON.stringify({ passed: true, approvedGlyphs: 1, strokes: 13, sourceVersions: versions, privateGraphicsSaved: false }))
