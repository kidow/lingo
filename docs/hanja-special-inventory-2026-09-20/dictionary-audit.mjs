...
export async function auditDictionary({ retryInventory = null } = {}) {
  const catalogPath = 'content/hanja/characters/special.json'
  const bytes = readFileSync(root + catalogPath)
  const catalog = JSON.parse(bytes)
  assert.equal(catalog.characters.length, 1328)
  ...
  return {
    schemaVersion: 1, checkedAt: '2026-09-20', purpose: 'Public dictionary availability and metadata inventory only; no visual stroke approval.',
    catalog: { file: catalogPath, sha256: hash(bytes), characters: catalog.characters.length, strokes: 18270 },
