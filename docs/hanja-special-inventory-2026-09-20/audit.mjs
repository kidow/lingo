/** Read-only source/count inventory for the special grade. Never approves strokes or writes downloaded assets. */
...
import { CANDIDATE_SOURCE, JAPANESE_CANDIDATE_SOURCE, MAKE_ME_A_HANZI_SOURCE, TRADITIONAL_CANDIDATE_SOURCE,
  SIMPLIFIED_CANDIDATE_SOURCE, parseCandidates } from '../../scripts/hanja-stroke-audit.ts'

const root = fileURLToPath(new URL('../../', import.meta.url))
const read = path => readFileSync(new URL('../../' + path, import.meta.url))
const hash = bytes => createHash('sha256').update(bytes).digest('hex')
const json = path => JSON.parse(read(path))
// 특급II 조사는 셋(MM·Ja·Ko)만 봤다. 그 뒤 같은 AnimCJK 리비전의 ZhHant·ZhHans를 후보에 넣었으므로 처음부터 다섯을 본다
export const CORPUS_IDS = ['MM', 'Ja', 'Ko', 'Hant', 'Hans']
...
export async function audit() {
  const catalogFile = 'content/hanja/characters/special.json', catalogBytes = read(catalogFile)
  const catalog = JSON.parse(catalogBytes), chars = catalog.characters
  assert.equal(chars.length, 1328)
  assert.equal(new Set(chars.map(c => c.glyph)).size, 1328)
  assert.ok(chars.every(c => c.readingGrade === '특급'))

  // 교과서 목록은 고등 한문 1,800자다. 특급 겹침은 audit 결과로 확인하고, 겹치는 글자만 영상 존재를 확인한다.
  // 교과서 경로는 사전보다 앞선 출처라 따로 남긴다
  ...
  const rows = chars.map((c, i) => {
    ...
    const counts = CORPUS_IDS.map(id => {
    ...
    return [c.glyph, c.strokes, c.radical, s?.glyph ?? null, s?.manifestRow ?? null, match, ...counts, video?.status ?? null, video?.bytes ?? null, video?.type ?? null, video?.error ?? null]
  })

  const geometryGroups = { countMatch: [], countReview: [], missing: [] }
  for (const row of rows) {
    const counts = row.slice(6, 6 + CORPUS_IDS.length)
  ...
  const corpusCoverage = Object.fromEntries(CORPUS_IDS.map((id, i) => [id, {
    available: rows.filter(r => r[6 + i] !== null).length, countMatch: rows.filter(r => r[6 + i] === r[1]).length,
  }]))
  const runtimeCount = chars.filter(c => hanjaStrokeData(c)).length
  return {
    version: 1, date: '2026-09-20', purpose: 'Source and candidate-count discovery only; no per-stroke visual review or runtime approval.',
    ...
    fields: ['glyph', 'catalogStrokes', 'radical', 'publisherGlyph', 'publisherRow', 'publisherMatch', ...CORPUS_IDS.map(id => id + 'Strokes'),
