...
const runtime=hanjaStrokeData({glyph:'扤',strokes:6})
assert(runtime);assert.equal(runtime.verificationSource,'ehanja-crosschecked')
assert.equal(proof.groups.length,6);assert.equal(sourceGroups.length,6)
assert.deepEqual(sourceGroups.flat().sort((a,b)=>a-b),Array.from({length:6},(_,i)=>i))
...
const editable=readFileSync(new URL('../../public/hanja-strokes/glyphwiki/6264.json',import.meta.url))
...
for(const i of [1,4,5])assert.deepEqual(trace[i][0].args.slice(2,4),trace[i][1].args.slice(0,2))
assert.deepEqual(trace[5][1].args.slice(4,6),trace[5][2].args.slice(0,2))
...
console.log(JSON.stringify({passed:true,sourceVersions,engineChecked,rawGroups:6,strokeGroups:6,curvedStrokes:4,runtimeRegistered:1,privateMediaSaved:false}))
