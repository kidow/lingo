...
assert.equal(hanjaStrokeData({glyph:'蓑',strokes:14}),null)
assert.equal(proof.groups.length,16)
assert.deepEqual(trace[12][0].args.slice(2,4),[66,175.8])
assert.deepEqual(trace[13][0].args.slice(0,2),[37,180.6])
assert.notDeepEqual(trace[12][0].args.slice(2,4),trace[13][0].args.slice(0,2))
assert.deepEqual(trace[15][0].args.slice(0,2),[106,121.4])
assert.equal(trace[10][0].args[1],121.4)
const alternatives=read('./alternatives.json')
assert.equal(alternatives.records['u84d1'].data,'99:0:0:0:0:200:200:u84d1-j')
assert.equal(alternatives.records['u8870-j'].data,source.records['u8870-j'].data)
assert.equal(alternatives.records['u8279-03'].data.split('$').length,3)
const findings=read('./findings.json')
assert.deepEqual(findings.reviewedCumulativeStrokes,Array.from({length:14},(_,i)=>i+1))
assert.equal(findings.fullStrokeApproval,false)
const {makeRequest,validateResponse}=await import('../../scripts/hanja-jev-review.mjs')
...
console.log(JSON.stringify({passed:true,sourceVersions,engineChecked,rawGroups:16,domesticStrokes:14,runtimeRegistered:0,held:true,privateMediaSaved:false}))
