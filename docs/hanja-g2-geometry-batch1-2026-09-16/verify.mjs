...
import {HANJA_DICTIONARY_G2_GEOMETRY_BATCH1_STROKES} from '../../lib/hanja-stroke-dictionary-g2-geometry-batch1.ts'
import {buildG2GeometryBatch1DictionaryBundle,validateG2GeometryBatch1DictionaryBundle} from '../../scripts/hanja-stroke-dictionary-g2-geometry-batch1.ts'
...
assert.deepEqual(read('../../public/hanja-strokes/dictionary-reviewed-g2-geometry-batch1.json'),prepared.runtime)
assert.deepEqual(buildG2GeometryBatch1DictionaryBundle(),prepared.runtime)
validateG2GeometryBatch1DictionaryBundle()
assert.deepEqual(originals.entries.map(e=>[e.glyph,e.strokes,e.catalogStrokes,e.dictionaryStrokes]),[['葛',12,13,13],['儆',14,15,15],['菓',11,12,12]])
assert.equal(corrections.automaticRadicalApproval,false)
assert.equal(corrections.edits.length,14)
...
const short=sources.entries.find(e=>e.glyph==='儆').shortHorizontalDirectionChecks
assert.deepEqual(short.map(r=>r.stroke),[3,5,11]);assert.ok(short.every(r=>r.leftToRight && r.commands.join('')==='ML'))
...
const previous=read('../hanja-g2-yu-2026-09-16/progress.json')
...
assert.deepEqual(nextGlyphs,['董','藤','鄧'])
...
const nextBatch={schemaVersion:1,title:'董·藤·鄧 3자·'+nextStrokes+'획 경로 획수 차이 검토',characters:3,strokes:nextStrokes,
...
const progress={date:'2026-09-16',batch:{sourceAudited:3,reviewedStrokes:40,applied:3,appliedStrokes:40,held:0,authoredOrCorrectedPaths:14},
...
for(const entry of HANJA_DICTIONARY_G2_GEOMETRY_BATCH1_STROKES){
