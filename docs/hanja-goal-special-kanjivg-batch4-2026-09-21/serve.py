"""Special batch 4: original rendering width comparison; private graphics remain in RAM."""
import sys
...
def candidate(entry, end=None):
    width = "3" if entry["viewBox"] == "0 0 109 109" else str(300 / 109)
    return original_candidate(entry, end).replace('stroke-width="5"', 'stroke-width="' + width + '"').replace('stroke-width="4"', 'stroke-width="' + width + '"')
