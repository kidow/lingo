...
assert.equal(actual.reduce((n,e) => n + e.strokes, 0), 35)
...
assert.equal(next.remainingCountMatched.length, 17)
assert.equal(new Set(next.remainingCountMatched.map(e=>e.glyph)).size,17)
assert(next.remainingCountMatched.every(e=>!metadata.some(m=>m.glyph===e.glyph)))
console.log(JSON.stringify({passed:true,publicSourceAndPointHashes:true,normalizedPathHashes:3,domesticVisualSteps:35,newRuntimeGlyphs:0,nextCountMatchedGlyphs:17}))
