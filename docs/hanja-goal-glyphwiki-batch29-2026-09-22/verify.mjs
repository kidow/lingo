...
const expected = read('public/hanja-strokes/dictionary-reviewed-glyphwiki-batch29.json')
...
assert.deepEqual(hanjaStrokeData({ glyph: '芒', strokes: 7 })?.paths, built[0].paths)
assert.equal(hanjaStrokeData({ glyph: '芒', strokes: 8 }), null)
let versions = 0
if (process.argv.includes('--sources')) {
  const source = read('public/hanja-strokes/glyphwiki/8292.json')
...
console.log(JSON.stringify({ passed: true, approvedGlyphs: 1, strokes: 7, sourceVersions: versions, privateGraphicsSaved: false }))
