#!/usr/bin/env python3
"""Pinned batch3 follow-up comparison. Source videos and decoded frames stay in RAM."""
import base64
import hashlib
import html
...
QUEUE = json.loads((HERE.parent / 'hanja-g3-batch3-2026-09-14/queue.json').read_text())
HELD = set('朋祥暑逝涉蔬睡遂尋餓')
QUEUE['entries'] = [r for r in QUEUE['entries'] if r['glyph'] in HELD]
    ...
    def do_GET(self):
        try:
            ...
            if glyph not in BASE.QUEUE:
                raise ValueError('Glyph outside the fixed 10')
            r = BASE.load(glyph)
...
if __name__ == '__main__':
    server = ThreadingHTTPServer(('127.0.0.1', 51719), Handler)
    print(f'G3 batch3 follow-up: http://127.0.0.1:{server.server_port}', flush=True)
