...
const expected = [...'朋祥暑逝涉蔬睡遂尋餓']
const queue = json('../hanja-g3-batch3-2026-09-14/queue.json')
const originals = json('../hanja-g3-batch3-2026-09-14/originals.json')
const oldObservations = json('../hanja-g3-batch3-2026-09-14/observations.json').records
...
const sources = json('./sources.json'), generated = prepare()
...
assert.deepEqual(sources.videos.map(e => e.glyph), expected)
...
assert.equal(review.records.reduce((n, e) => n + e.expectedStrokes, 0), 123)
assert.deepEqual(sources.geometry, queue.geometry.MM)
...
for (const [glyph, first] of [['逝', 8], ['遂', 10]]) {
  const [a, b] = proposals[glyph].slice(first, first + 2)
  assert.equal(a.sourceStroke, b.sourceStroke)
  assert.deepEqual(a.points.at(-1), b.points[0])
}
...
console.log(JSON.stringify({
  result: 'pass', reviewedCharacters: 10, reviewedStrokes: 123,
  addedCharacters: 10, addedStrokes: 123, corrections: 10, heldCharacters: 0,
