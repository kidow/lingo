...
export async function acquire() {
  const targets = [['塏',13,13,'Components'],['璟',16,16,'ZhHans'],['琯',12,12,'ZhHans']]
  ...
  const components = [{glyph:'土',targetBox:[9,12,27,76]},{glyph:'豈',targetBox:[43,10,48,80]}].map(part => {
  ...
  const composed = {character:'塏',medians:compose(components)}
  composed.strokes = normalizeMedians(composed.medians)
  sources.Components = {url:'docs/hanja-g2-geometry-batch14-2026-09-16/originals.json#components',
    sha256:hash(JSON.stringify(components)),license:sources.MM.license,bytes:Buffer.byteLength(JSON.stringify(components))}
  candidates.Components = [composed]
  const entries = []
  for (const [glyph,catalogStrokes,expected,preferred] of targets) {
    ...
    entries.push({glyph,strokes:candidate.medians.length,catalogStrokes,dictionaryStrokes:expected,corpus,sourceGlyph:candidate.character,alternatives,
      ...(corpus==='Components'?{components}:{}),medians:candidate.medians,originalMediansSha256:hash(JSON.stringify(candidate.medians)),paths:normalizeMedians(candidate.medians),
  ...
  return {schemaVersion:1,date:'2026-09-16',purpose:'Unapproved 塏 璟 琯 alternate-corpus and component-based candidates and refreshed whole-glyph dictionary sources; review every boundary and direction before registration.',
    sources,availability,entries,characters:3,strokes:entries.reduce((n,e)=>n+e.strokes,0),catalogStrokes:41,dictionaryStrokes:41,proprietaryAssetsSaved:0,runtimeApprovalsAdded:0}
}
