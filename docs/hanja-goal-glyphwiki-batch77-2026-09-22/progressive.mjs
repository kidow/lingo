...
export function compileProgressive(trace, proof) {
  ...
  return sourceGroups.map(indices=>indices.flatMap(index=>{
    ...
    return trace[index].flatMap((call,j)=>{
      ...
      if(index===7&&j===2){
        assert.deepEqual(a,[142.84,172,182.12,172,6,5])
        assert.equal(call.polygons.length,3)
        assert.deepEqual(call.polygons[2].map(p=>[p.x,p.y]),[[182.1,166],[184.1,141],[182.1,141],[176.1,166]])
        return [segment(call.polygons.slice(0,2),'right',(x2-x1)/2),segment(call.polygons.slice(2),'up',(166-141)/2)]
      }
