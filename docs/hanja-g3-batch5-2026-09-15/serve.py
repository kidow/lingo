...
BASE.QUEUE = {r['glyph']: r for r in QUEUE['entries']}
...
def load(glyph):
    if BASE.QUEUE[glyph]['candidate'] is not None:
        return BASE.load(glyph)
    with BASE.LOCK:
        if glyph not in BASE.CACHE:
            evidence = BASE.QUEUE[glyph]['sourceVideo']
            original = BASE.download(evidence['url'], 20_000_000)
            assert len(original) == evidence['bytes']
            assert hashlib.sha256(original).hexdigest() == evidence['sha256']
            data = BASE.MEMORY.faststart(original)
            probe = json.loads(BASE.MEMORY.run(['ffprobe', '-v', 'error', '-i', 'pipe:0', '-show_entries', 'format=duration:stream=codec_type,width,height', '-of', 'json'], data))
            stream = next(s for s in probe['streams'] if s['codec_type'] == 'video')
            assert (stream['width'], stream['height']) == (1920, 1080)
            BASE.CACHE[glyph] = {'data': data, 'originalMedians': [], 'points': [], 'sourceVideo': evidence, 'durationSeconds': float(probe['format']['duration'])}
        return BASE.CACHE[glyph]
    ...
    def do_GET(self):
        try:
            ...
            if glyph not in BASE.QUEUE:
                raise ValueError('Glyph outside the fixed 50')
            r = load(glyph)
            item = BASE.QUEUE[glyph]
            if self.path.startswith('/metadata'):
                out = {k: v for k, v in r.items() if k != 'data'}
                out.update(item)
                out['geometrySource'] = QUEUE['geometry'].get(item['candidate'])
                ...
                if view == 'source':
                    ...
                    content += '<p>Every cumulative path · green start / red end · width 5</p>' + (BASE.candidate_html(points) if points else '<p>No licensed candidate available</p>')
                else:
...
if __name__ == '__main__':
    server = ThreadingHTTPServer(('127.0.0.1', 51726), Handler)
    print(f'G3 batch5: http://127.0.0.1:{server.server_port}', flush=True)
