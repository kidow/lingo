...
import {HANJA_DICTIONARY_G2_GEOMETRY_BATCH2_STROKES} from '../../lib/hanja-stroke-dictionary-g2-geometry-batch2.ts'
import {buildG2GeometryBatch2DictionaryBundle,validateG2GeometryBatch2DictionaryBundle} from '../../scripts/hanja-stroke-dictionary-g2-geometry-batch2.ts'
...
assert.deepEqual(read('../../public/hanja-strokes/dictionary-reviewed-g2-geometry-batch2.json'),prepared.runtime)
assert.deepEqual(buildG2GeometryBatch2DictionaryBundle(),prepared.runtime)
validateG2GeometryBatch2DictionaryBundle()
assert.deepEqual(originals.entries.map(e=>[e.glyph,e.strokes,e.catalogStrokes,e.dictionaryStrokes]),[['董',12,13,13],['藤',18,19,19],['鄧',14,15,15]])
assert.equal(corrections.automaticRadicalApproval,false)
assert.equal(corrections.edits.length,28)
...
const previous=read('../hanja-g2-geometry-batch1-2026-09-16/progress.json')
...
assert.equal(queues.geometryCountReview.strokes,previous.remainingGrade2Queues.geometryCountReview.strokes-47)
...
assert.deepEqual(nextGlyphs,['礪','遼','膜'])
...
const nextBatch={schemaVersion:1,title:'礪·遼·膜 3자·'+nextStrokes+'획 경로 획수 차이 검토',characters:3,strokes:nextStrokes,
...
const progress={date:'2026-09-16',batch:{sourceAudited:3,reviewedStrokes:47,applied:3,appliedStrokes:47,held:0,authoredOrCorrectedPaths:28},
...
for(const entry of HANJA_DICTIONARY_G2_GEOMETRY_BATCH2_STROKES){
