...
const runtime=hanjaStrokeData({glyph:'芐',strokes:7})
...
const editable=readFileSync(new URL('../../public/hanja-strokes/glyphwiki/8290.json',import.meta.url))
...
assert.deepEqual(sourceGroups,[[0],[1],[3],[2],[4],[5],[6]])
assert.equal(trace[4][0].args[1],trace[5][0].args[1])
assert.equal(trace[5][0].args[0],trace[6][0].args[0])
...
console.log(JSON.stringify({passed:true,sourceVersions,engineChecked,rawGroups:7,strokeGroups:7,curvedStrokes:1,runtimeRegistered:1,privateMediaSaved:false}))
