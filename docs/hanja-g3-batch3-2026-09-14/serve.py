#!/usr/bin/env python3
"""Pinned batch3 comparison, including explicitly held count-mismatch candidates. Source videos and decoded frames stay in RAM."""
import base64
import hashlib
import html
        ...
        try:
            ...
            if self.path.startswith('/metadata'):
                ...
                if q.get('corrected', ['0'])[0] == '1' and orders_file.exists():
                    order = json.loads(orders_file.read_text()).get(glyph, list(range(1, len(points) + 1)))
                    if sorted(order) != list(range(1, len(points) + 1)):
                        raise ValueError('Not a stroke permutation')
                ...
                if view == 'source':
                    ...
                    content += '<svg width="420" height="420" viewBox="0 0 100 100">' + ''.join(f'<path d="{path}" fill="none" stroke="#222" stroke-width="5" stroke-linecap="round" stroke-linejoin="round"/>' for path in paths) + '</svg>'
...
if __name__ == '__main__':
    server = ThreadingHTTPServer(('127.0.0.1', 51718), Handler)
    print(f'G3 batch3 review: http://127.0.0.1:{server.server_port}', flush=True)
