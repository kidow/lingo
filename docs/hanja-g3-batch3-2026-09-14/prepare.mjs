...
export function prepare() {
  ...
  for (const observation of observations.filter(o => o.decision === 'matched')) {
    ...
    const recipe = observation.order ? {
      glyph: item.glyph, id: `vivasam-g3-batch3-${item.glyph.codePointAt(0).toString(16)}-order-v1`,
    ...
    const record = {
      ...
      notes: observation.notes + ' Evidence and holds: docs/hanja-g3-batch3-2026-09-14/. Times are sampled completed-by observations, not exact pen lifts. Only this whole-glyph comparison authorizes this entry.',
    }
