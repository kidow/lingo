...
const expected = read('public/hanja-strokes/dictionary-reviewed-glyphwiki-batch36.json')
...
assert.deepEqual(hanjaStrokeData({ glyph: '萃', strokes: 12 })?.paths, built[0].paths)
assert.equal(hanjaStrokeData({ glyph: '萃', strokes: 13 }), null)
let versions = 0
if (process.argv.includes('--sources')) {
  const source = read('public/hanja-strokes/glyphwiki/8403.json')
...
console.log(JSON.stringify({ passed: true, approvedGlyphs: 1, strokes: 12, sourceVersions: versions, privateGraphicsSaved: false }))
