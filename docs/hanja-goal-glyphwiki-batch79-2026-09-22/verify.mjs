...
const runtime=hanjaStrokeData({glyph:'杝',strokes:7})
...
const editable=readFileSync(new URL('../../public/hanja-strokes/glyphwiki/675d.json',import.meta.url))
...
assert.deepEqual(trace[4][0].args.slice(2,4),trace[7][0].args.slice(0,2))
...
assert.deepEqual(trace[6][1].args.slice(4,6),trace[6][2].args.slice(0,2))
...
console.log(JSON.stringify({passed:true,sourceVersions,engineChecked,rawGroups:8,strokeGroups:7,curvedStrokes:4,runtimeRegistered:1,privateMediaSaved:false}))
