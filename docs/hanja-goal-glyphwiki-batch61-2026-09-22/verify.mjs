...
const runtime=hanjaStrokeData({glyph:'玘',strokes:7})
assert(runtime);assert.equal(runtime.verificationSource,'ehanja-crosschecked')
assert.equal(proof.groups.length,8);assert.equal(sourceGroups.length,7)
assert.deepEqual(sourceGroups.flat().sort((a,b)=>a-b),Array.from({length:8},(_,i)=>i))
...
const editable=readFileSync(new URL('../../public/hanja-strokes/glyphwiki/7398.json',import.meta.url))
...
assert.equal(asset.engineStyle,'mincho');assert.equal(asset.engineConstructorSize,'default')
assert.deepEqual(outlines[6].map(s=>s.direction),['down','curve','right','up'])
assert.deepEqual(trace[7][0].args.slice(2,4),trace[7][1].args.slice(0,2))
assert.deepEqual(trace[7][1].args.slice(4,6),trace[7][2].args.slice(0,2))
assert.deepEqual(read('./progressive-review.json').reviewedStrokes,Array.from({length:7},(_,i)=>i+1))

const points=path=>[...path.matchAll(/[ML](-?[0-9.]+) (-?[0-9.]+)/g)].map(m=>[Number(m[1]),Number(m[2])])
for(let i=0;i<sourceGroups.length;i++){
 const originals=sourceGroups[i].flatMap(index=>trace[index].flatMap(call=>call.polygons))
 const parts=outlines[i].flatMap(segment=>segment.outline.split(' Z').map(p=>p.trim()).filter(Boolean))
 assert.equal(parts.length,originals.length)
 for(let j=0;j<parts.length;j++){
  const actual=points(parts[j]),expected=originals[j].map(p=>[p.x/2,p.y/2])
  assert.deepEqual(actual.map(p=>p.join(',')).sort(),expected.map(p=>p.join(',')).sort())
  assert(actual.reduce((n,p,k)=>{const next=actual[(k+1)%actual.length];return n+p[0]*next[1]-next[0]*p[1]},0)>0)
 }
}
...
console.log(JSON.stringify({passed:true,sourceVersions,engineChecked,rawGroups:8,strokeGroups:7,hookSegments:4,runtimeRegistered:1,privateMediaSaved:false}))
