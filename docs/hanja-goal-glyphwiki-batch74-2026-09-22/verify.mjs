...
const runtime=hanjaStrokeData({glyph:'伻',strokes:7})
assert(runtime);assert.equal(runtime.verificationSource,'ehanja-crosschecked')
assert.equal(proof.groups.length,7);assert.equal(sourceGroups.length,7)
...
const editable=readFileSync(new URL('../../public/hanja-strokes/glyphwiki/4f3b.json',import.meta.url))
...
assert.deepEqual(trace[4][0].args.slice(0,6),[106.475,48,97.775,75,76.75,97])
assert.deepEqual(trace[3][0].args.slice(0,6),[146.35,48,165.925,69,173.175,93])
...
console.log(JSON.stringify({passed:true,sourceVersions,engineChecked,rawGroups:7,strokeGroups:7,curvedStrokes:3,runtimeRegistered:1,privateMediaSaved:false}))
