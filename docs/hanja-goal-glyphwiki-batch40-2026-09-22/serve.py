...
r.ENTRIES=[{'glyph':'薪','id':'glyphwiki-u85aa-k-width1p25','dictionaryStrokes':17,'dictionary':metadata['dictionary'],'viewBox':'0 0 100 100','strokes':[{'path':p}for p in paths]}]
r.base.renderer.ENTRIES['薪']={'strokes':17,'dictionary':metadata['dictionary']}
original=r.candidate
def candidate(entry,end=None):return original(entry,end).replace('stroke-width="5"','stroke-width="1.25"').replace('stroke-width="4"','stroke-width="1.25"')
...
print(json.dumps({'port':51893,'pid':os.getpid(),'primitives':len(paths),'boundaryPaths':paths[4:6]}),flush=True)
r.ThreadingHTTPServer(('127.0.0.1',51893),r.Handler).serve_forever()
