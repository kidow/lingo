#!/usr/bin/env python3
"""Compare 葺 source strokes without saving dictionary graphics."""
import sys
...
r.ENTRIES = [{'glyph':'葺','id':'glyphwiki-u847a-k-width1p5','dictionaryStrokes':13,
    'dictionary':meta['dictionary'],'viewBox':'0 0 100 100','strokeWidth':1.5,
    'strokes':[{'path':p} for p in approved['paths']]}]
r.base.renderer.ENTRIES['葺'] = {'strokes':13,'dictionary':meta['dictionary']}
original_candidate = r.candidate
def candidate(entry, end=None):
    return original_candidate(entry,end).replace('stroke-width="5"','stroke-width="1.5"').replace('stroke-width="4"','stroke-width="1.5"')
...
if __name__ == '__main__':
    port = int(sys.argv[1]) if len(sys.argv)>1 else 51884
