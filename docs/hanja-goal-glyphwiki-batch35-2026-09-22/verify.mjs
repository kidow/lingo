...
const expected = read('public/hanja-strokes/dictionary-reviewed-glyphwiki-batch35.json')
...
assert.deepEqual(hanjaStrokeData({ glyph: '葺', strokes: 13 })?.paths, built[0].paths)
assert.equal(hanjaStrokeData({ glyph: '葺', strokes: 14 }), null)
let versions = 0
if (process.argv.includes('--sources')) {
  const source = read('public/hanja-strokes/glyphwiki/847a.json')
