...
assert.equal(hanjaStrokeData({glyph:'葯',strokes:13}),null)
assert.equal(proof.groups.length,16)
assert.deepEqual(trace[15][0].args,[102,118.875,125,128.3,137,144.975,7,8])
assert.notDeepEqual(trace[4][0].args.slice(4,6),trace[5][0].args.slice(0,2))
assert.notDeepEqual(trace[6][0].args.slice(4,6),trace[7][0].args.slice(0,2))
const alternatives=read('./alternatives.json')
assert.equal(alternatives.records['u846f'].data,'99:0:0:0:0:200:200:u846f-j')
assert.equal(alternatives.records['u7d04-j@2'].data,source.records['u7d04-j@2'].data)
