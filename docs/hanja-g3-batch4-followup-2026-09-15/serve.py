#!/usr/bin/env python3
"""Pinned batch4 comparison. Source videos and decoded frames stay in RAM."""
import base64
import hashlib
import html
...
QUEUE = json.loads((HERE.parent / 'hanja-g3-batch4-2026-09-14/queue.json').read_text())
    ...
    def do_GET(self):
        try:
            ...
            if glyph not in BASE.QUEUE:
                raise ValueError('Glyph outside the fixed ten')
            r = BASE.load(glyph)
...
if __name__ == '__main__':
    server = ThreadingHTTPServer(('127.0.0.1', 51725), Handler)
    print(f'G3 batch4 follow-up: http://127.0.0.1:{server.server_port}', flush=True)
