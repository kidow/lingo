...
const expected = [...'躍閱銳傲翁遙腰違緯愈']
const queue = json('../hanja-g3-batch4-2026-09-14/queue.json')
const originals = json('../hanja-g3-batch4-2026-09-14/originals.json')
const oldObservations = json('../hanja-g3-batch4-2026-09-14/observations.json').records
...
assert.equal(review.records.reduce((n, e) => n + e.expectedStrokes, 0), 142)
...
for (const [glyph, first] of [['遙', 11], ['違', 10]]) {
...
assert.equal(proposals['遙'].some(s => s.sourceStroke === 12), false)
assert.equal(proposals['違'].some(s => s.sourceStroke === 9), false)
assert.deepEqual(proposals['傲'].map(s => s.sourceStroke), Array.from({ length: 13 }, (_, i) => i + 1))
...
console.log(JSON.stringify({
  result: 'pass', reviewedCharacters: 10, reviewedStrokes: 142,
  addedCharacters: 10, addedStrokes: 142, corrections: 10, heldCharacters: 0,
