...
const candidates = read('./candidate-paths.json')
const review = read('./review.json')
const sources = read('../hanja-g3ii-dictionary-12-2026-09-13/sources.json')
const observations = read('../hanja-g3ii-dictionary-12-2026-09-13/observations.json')
const glyphs = ['森', '楓']
...
for (const original of originals.entries) {
  const glyph = original.glyph
  const ref = DICTIONARY_REFERENCES[glyph]
  ...
  const rows = observations.rows.filter(row => row[0] === glyph)
  ...
  assert.deepEqual(normalizeMedians(original.medians), candidate.paths)
  ...
  assert.ok(rows.every(row => row[2] > 0 && row[2] < row[3] && row[4] === row[1] - 1 && row[5]))
...
console.log(JSON.stringify({ status: 'passed', glyphs, verifiedStrokes: 25,
  liveSourcesChecked, priorMovingObservations: 25, cumulativeStagesReviewed: 25,
