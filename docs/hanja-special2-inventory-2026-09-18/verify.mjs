...
const catalogBytes = read('../../content/hanja/characters/special-2.json'), catalog = JSON.parse(catalogBytes)
const expectedGlyphs = catalog.characters.map(c => c.glyph)
assert.equal(expectedGlyphs.length, 1150)
assert.equal(new Set(expectedGlyphs).size, 1150)
assert.equal(catalog.characters.reduce((n, c) => n + c.strokes, 0), 15002)
assert.ok(catalog.characters.every(c => c.readingGrade === '특급II'))

const records = snapshot => {
  assert.equal(snapshot.catalog.sha256, hash(catalogBytes))
  assert.equal(snapshot.catalog.characters, 1150)
  assert.equal(snapshot.catalog.strokes, 15002)
  assert.equal(snapshot.rows.length, 1150)
...
const verifyPartition = groups => {
  ...
  assert.equal(flat.length, 1150)
  assert.equal(new Set(flat).size, 1150)
...
assert.deepEqual(inventory.runtime, { allCharacters: expected.runtimeStrokes, gradeEnabled: 0, gradeRemaining: 1150, added: 0 })
...
console.log(JSON.stringify({ status: 'passed', records: 1150, strokes: 15002, dictionary: dictionary.counts,
