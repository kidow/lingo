...
확인·적용일: 2026-09-07. 대상: 현재 한능검 트랙의 고유 한자 5,978자. 웹의 제작자 문서와 실제 공개 폰트 파일을 확인한 뒤 표시용 SVG를 적용했다. 아래 기계 변환 검사는 최초 조사 기록이며 실제 적용 내용은 마지막 절에 기록한다.
...
**한국어용 공개 폰트의 윤곽을 정적 SVG로 추출해 글자 표시를 통일했다.** `public/hanja/`에는 한자 5,978자와 추가 부수 45자의 SVG, 글자별 해시가 있는 `manifest.json`, `OFL.txt`, `NOTICE.txt`가 있다. SVG마다 동일한 1000 단위 정사각형을 사용하고 원본 윤곽 크기를 보존하면서 가운데 정렬한다. 경계가 큰 글자만 여백 안에 들어오도록 축소한다. 원본 폰트 파일이나 파서는 앱에 넣지 않는다.

`HanjaGlyph`는 외부 SVG의 `#glyph`를 참조하므로 전체 경로 데이터를 JavaScript 번들로 가져오지 않는다. 소개, 훈음 퀴즈의 한자, 한자 선택 보기, 3열 검색 결과, 검색 상세, 부수, 예시 한자어와 쓰기 기준 글자에 적용한다. 접근성 이름을 유지하고 `currentColor`로 정답·오답·참조 색상을 따른다.

완성 글자 표시와 공식 검증 획 재생 데이터는 별도로 관리한다. 검증된 14자도 재생 전에는 Noto SVG를 표시하고, 재생을 시작한 뒤에는 기존 공식 도해 대조 경로를 사용한다. 표시용 SVG의 획을 추정해 애니메이션 대상으로 확대하지 않는다.

재현 도구 `scripts/hanja-svg.mjs`는 원본 폰트와 변환기 해시를 검사한 뒤 파일 내용의 JSON을 표준 출력으로 반환한다. 작업 공간을 직접 덮어쓰지 않는다. `node scripts/hanja-svg.mjs --verify`는 현재 자산과 원본에서 다시 생성한 결과가 완전히 같은지 검사한다. 단위 검사는 전체 글자·부수·예시의 자산 누락, 변조, 텍스트·스크립트 의존 SVG와 라이선스 누락을 검사한다.

2026-09-07 검증: SVG 6,023개가 고정한 원본 폰트·변환기로 재생성한 결과와 일치했다. 타입 검사, Next.js webpack 프로덕션 빌드, 전체 테스트 141개가 통과했다. 로컬 브라우저에서 4지선다 한자, 3열 찾기 결과, 특급 斝와 부수, 山의 1/3→3/3 획 재생, 쓰기 연습의 SVG 참고 글자와 색상 상속을 확인했다. 확인한 화면에서 콘솔 오류는 없었다.
