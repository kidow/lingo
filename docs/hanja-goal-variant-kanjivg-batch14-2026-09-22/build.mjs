...
console.log(JSON.stringify(entries.map(review => {
  ...
  if (review.rendering.sourceSha256 !== entry.candidate.sha256 || review.rendering.normalizedStrokeWidth !== ({ "祉": 5 })[entry.glyph]) throw new Error("Unreviewed rendering width")
  if (entry.catalogStrokes !== 9 || entry.dictionaryStrokes !== 8 || JSON.stringify(review.variant) !== JSON.stringify({ catalogStrokes: 9, playbackStrokes: 8, form: "사전" })) throw new Error("Unreviewed playback variant")
