...
export function prepare() {
  ...
  for (const observation of json('./observations.json').filter(o => o.decision === 'matched')) {
    ...
    const recipe = proposals[item.glyph] ? {
      glyph: item.glyph, id: `vivasam-g3-batch6-followup-${item.glyph.codePointAt(0).toString(16)}-v1`,
    ...
    const record = {
      ...
      notes: observation.notes + ' Evidence: docs/hanja-g3-batch6-followup-2026-09-15/. Times are sampled completed-by observations, not exact pen lifts. Only this whole-glyph comparison authorizes this entry.',
    }
