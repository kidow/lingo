...
import {HANJA_DICTIONARY_G2_GEOMETRY_BATCH3_STROKES} from '../../lib/hanja-stroke-dictionary-g2-geometry-batch3.ts'
import {buildG2GeometryBatch3DictionaryBundle,validateG2GeometryBatch3DictionaryBundle} from '../../scripts/hanja-stroke-dictionary-g2-geometry-batch3.ts'
...
assert.deepEqual(read('../../public/hanja-strokes/dictionary-reviewed-g2-geometry-batch3.json'),prepared.runtime)
assert.deepEqual(buildG2GeometryBatch3DictionaryBundle(),prepared.runtime)
validateG2GeometryBatch3DictionaryBundle()
assert.deepEqual(originals.entries.map(e=>[e.glyph,e.strokes,e.catalogStrokes,e.dictionaryStrokes]),[['礪',19,20,20],['遼',15,16,16],['膜',14,15,15]])
assert.equal(corrections.automaticRadicalApproval,false)
assert.equal(corrections.edits.length,39)
...
for(const glyph of ['礪','遼']){
 const contact=sources.entries.find(e=>e.glyph===glyph).contactChecks[0]
 assert.equal(contact.sourceBoundaryGapUnits,0);assert.equal(contact.sourceArtworkSaved,false)
}
const progression=sources.entries.find(e=>e.glyph==='遼').pathProgressionChecks[0]
assert.equal(progression.stroke,16)
assert.deepEqual(progression.segments.map(s=>s.x+':'+s.screenY),['right:up','right:down','right:down','right:up','right:up'])
...
const previous=read('../hanja-g2-geometry-batch2-2026-09-16/progress.json')
...
assert.equal(queues.geometryCountReview.strokes,previous.remainingGrade2Queues.geometryCountReview.strokes-51)
...
assert.deepEqual(nextGlyphs,['蔑','茅','范'])
...
const nextBatch={schemaVersion:1,title:'蔑·茅·范 3자·'+nextStrokes+'획 경로 획수 차이 검토',characters:3,strokes:nextStrokes,
...
const progress={date:'2026-09-16',batch:{sourceAudited:3,reviewedStrokes:51,applied:3,appliedStrokes:51,held:0,authoredOrCorrectedPaths:39},
...
assert.equal(progress.overall.appliedStrokes,previous.overall.appliedStrokes+51)
for(const entry of HANJA_DICTIONARY_G2_GEOMETRY_BATCH3_STROKES){
...
console.log(JSON.stringify({verification:'passed',reconstructionAndPublicationValid:true,allFiftyOneStagesObserved:true,catalogUnchanged:true,
