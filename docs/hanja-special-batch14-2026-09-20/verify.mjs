...
import { validateSpecialBatch14DictionaryBundle } from '../../scripts/hanja-stroke-dictionary-special-batch14.ts'
...
assert.deepEqual(read('../../public/hanja-strokes/dictionary-reviewed-special-batch14.json'), compiled.runtime)
...
const progress = {
  date: '2026-09-20', batch: { reviewed: 30, reviewedStrokes: compiled.review.reviewedStrokes,
  ...
  // The queue is empty once every eligible special grade character has been reviewed.
  next: entries.length === 0
    ? { task: 'No eligible special grade characters remain: the whole-stroke review queue is empty',
        first: null, last: null, characters: 0, strokes: 0,
        eligibleUnreviewedCharacters: 0, heldOutOfQueue: [...heldElsewhere] }
    : { task: 'Review the next 50 eligible special grade characters', first: entries[0].glyph,
...
console.log(JSON.stringify({ verification: 'passed', compilerMatchesArtifacts: true, proofPinsValid: true,
  uniqueRuntimeGlyphs: true, specialBatch14ApprovalsComplete: true, progress, nextBatch }, null, 2))
