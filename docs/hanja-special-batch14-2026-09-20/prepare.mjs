...
if (observations.status !== 'complete' || observations.entries.length !== 30
  || accepted.length + held.length !== 30 || accepted.length !== 30
...
const review = {
  ...
  reviewedCharacters: 30, reviewedStrokes: originals.entries.reduce((n, e) => n + e.strokes, 0),
...
const verification = {
  ...
  scope: `Special grade batch 14: 30 characters individually checked, ${accepted.length} approved and ${held.length} held; ${corrected} corrected forms rechecked. Not exam-body certification.`,
}
...
const characters = accepted.map(e => {
  ...
  return {
    ...
    geometryCorrection: 'dictionary-special-batch14-' + e.glyph.codePointAt(0).toString(16) + '-v1',
