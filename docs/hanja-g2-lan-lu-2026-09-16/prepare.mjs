...
if (basis.officialGrass.sha256 !== '4e191bbee54edd6db595f16fc83a15b9929eba0e3e01095da1e828c70e10760c'
  ...
  || basis.officialGrass.sourceRow !== 7
  || !basis.officialGrass.paragraph.includes('4획')
  || !isDeepStrictEqual(basis.entries.map(e => [e.glyph,e.strokes,e.dictionaryStrokes]), [['藍',18,17],['蘆',20,19]]))
  throw Error('Official grass/count basis changed')
for (const entry of basis.entries) {
  const character = catalog.find(c => c.glyph === entry.glyph)
  if (!character || character.strokes !== entry.strokes || entry.licensedOriginalStrokes !== entry.strokes
    ...
    throw Error('Catalog or explicit stage mapping changed')
}
const accepted = observations.entries.filter(e => e.decision === 'matched')
if (observations.status !== 'complete' || accepted.length !== 2 || observations.entries.length !== 2)
...
for (const observation of accepted) {
  ...
  if (!isDeepStrictEqual(source.strokes.map(s => s.xmlIndex),
      [original.dictionaryStrokes-2,original.dictionaryStrokes-1,original.dictionaryStrokes,...Array.from({length:original.dictionaryStrokes-3},(_,i)=>i+1)])
    || !isDeepStrictEqual(sourceChecks.officialGrass, basis.officialGrass)) throw Error('Source stage mapping changed')
...
const review = {
  ...
  reviewedCharacters: 2, reviewedStrokes: 38, approvedCharacters: 2, approvedStrokes: 38,
...
const characters = accepted.map(e => {
  ...
  return {
    ...
    geometryCorrection: 'dictionary-g2-lan-lu-' + e.glyph.codePointAt(0).toString(16) + '-v1',
    sourceStrokeIndices: proposals[e.glyph].map(p => p.sourceStroke), pathsSha256: hash(paths),
    sourceReference: {
      orderUrl: basis.officialGrass.url, dictionarySvgUrl: original.dictionary.url,
      dictionarySvgSha256: original.dictionary.sha256,
      dictionaryDirectionStrokes: Array.from({ length: original.dictionaryStrokes }, (_, i) => i + 1).join(','),
