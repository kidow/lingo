    ...
    def do_GET(self):
        try:
            ...
            if view == 'checks':
                ...
                VISITS.append({'view': view, 'glyphs': [e['glyph'] for e in selected], 'corpus': query.get('corpus', ['Ja'])[0]})
                ...
                VISITS.append({'view': view, 'glyph': glyph, 'start': start, 'end': end, 'corpus': entry['corpus']})
                body = ''.join(parts).encode()
                content_type = 'text/html; charset=utf-8'
...
if __name__ == '__main__':
    ThreadingHTTPServer(('127.0.0.1', int(sys.argv[1]) if len(sys.argv) > 1 else 51822), Handler).serve_forever()
