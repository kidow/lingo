#!/usr/bin/env python3
"""Compare 菩 source strokes without saving dictionary graphics."""
import sys
...
r.ENTRIES = [{'glyph':'菩','id':'glyphwiki-u83e9-k-width2','dictionaryStrokes':12,
    'dictionary':meta['dictionary'],'viewBox':'0 0 100 100','strokeWidth':2,
    'strokes':[{'path':p} for p in approved['paths']]}]
r.base.renderer.ENTRIES['菩'] = {'strokes':12,'dictionary':meta['dictionary']}
original_candidate = r.candidate
def candidate(entry, end=None):
    return original_candidate(entry,end).replace('stroke-width="5"','stroke-width="2"').replace('stroke-width="4"','stroke-width="2"')
...
if __name__ == '__main__':
    port = int(sys.argv[1]) if len(sys.argv)>1 else 51882
