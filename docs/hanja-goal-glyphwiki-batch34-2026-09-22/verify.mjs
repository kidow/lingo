...
const expected = read('public/hanja-strokes/dictionary-reviewed-glyphwiki-batch34.json')
...
assert.deepEqual(hanjaStrokeData({ glyph: '菩', strokes: 12 })?.paths, built[0].paths)
assert.equal(hanjaStrokeData({ glyph: '菩', strokes: 13 }), null)
let versions = 0
if (process.argv.includes('--sources')) {
  const source = read('public/hanja-strokes/glyphwiki/83e9.json')
