...
export function compileProgressive(trace, proof) {
  ...
  return sourceGroups.map(indices=>indices.flatMap(index=>{
    ...
    return trace[index].flatMap((call,j)=>{
      ...
      if(index===7&&j===2){
        assert.deepEqual(a,[50,176.44,176,176.44,6,5])
        assert.equal(call.polygons.length,3)
        assert.deepEqual(call.polygons[2].map(p=>[p.x,p.y]),[[176,170.4],[178,145.4],[176,145.4],[170,170.4]])
        return [segment(call.polygons.slice(0,2),'right',(x2-x1)/2),segment(call.polygons.slice(2),'up',(170.4-145.4)/2)]
      }
