...
import { validateG1Batch24DictionaryBundle } from '../../scripts/hanja-stroke-dictionary-g1-batch24.ts'
...
assert.deepEqual(read('../../public/hanja-strokes/dictionary-reviewed-g1-batch24.json'), compiled.runtime)
...
const progress = {
  date: '2026-09-20', batch: { reviewed: 16, reviewedStrokes: compiled.review.reviewedStrokes,
  ...
  // The eligible grade 1 queue is empty after this batch: report that plainly instead of a next range.
  next: { task: entries.length ? 'Review the next 50 eligible grade 1 characters' : 'No eligible grade 1 characters remain; the rest lack a licensed candidate with a matching stroke count or are held',
...
console.log(JSON.stringify({ verification: 'passed', compilerMatchesArtifacts: true, proofPinsValid: true,
  uniqueRuntimeGlyphs: true, g1Batch24ApprovalsComplete: true, progress, nextBatch }, null, 2))
