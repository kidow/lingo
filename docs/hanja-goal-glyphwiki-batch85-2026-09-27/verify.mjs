...
assert.equal(source.root,'u709a');assert.deepEqual(source.missing,[])
assert.equal(Object.keys(source.records).length,4);assert.equal(proof.groups.length,9)
...
assert.deepEqual(trace[8][0].args.slice(0,2),[133.625,59])
assert.equal(trace[5][0].args[1],51)
assert(trace[8][0].args[1]>trace[5][0].args[1])
assert.equal(read('./alternative-sources.json').records['u709a-k'].data,'99:0:0:0:0:200:200:u709a')
assert.equal(hanjaStrokeData({glyph:'炚',strokes:8}),null)
...
console.log(JSON.stringify({passed:true,sourceVersions,engineChecked,rawGroups:9,reviewedDomesticStrokes:8,finalStrokeRelativeStartMismatch:true,runtimeAdded:0,privateMediaSaved:false}))
