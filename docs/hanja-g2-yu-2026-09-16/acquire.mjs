...
export async function acquire() {
  ...
  for (const [glyph,catalogStrokes,expected] of targets) {
    const candidate = candidates.MM.find(c=>c.character===glyph)
    assert.equal(candidate.medians.length,catalogStrokes)
    ...
    entries.push({glyph,strokes:catalogStrokes,catalogStrokes,dictionaryStrokes:expected,corpus:'MM',
  ...
  return {schemaVersion:1,date:'2026-09-16',purpose:'Unapproved eleven-stroke 庾 candidate; compare the complete eleven and twelve-stroke source forms.',
    sources,availability,entries,characters:1,strokes:11,catalogStrokes:11,dictionaryStrokes:12,proprietaryAssetsSaved:0,runtimeApprovalsAdded:0}
}
