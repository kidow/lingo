...
기존 Web Animations 재생기를 유지하고 Motion 의존성은 추가하지 않았다. 공식 누적 도해를 추가 대조한 校·敎·九·兄의 자체 중심선을 등록해 재생 가능한 글자가 18자가 됐다. 원문 위치와 대조 범위는 [필순 검증 목록](hanja-stroke-sources.md#현재-적용한-18자)에 기록한다.

`node scripts/hanja-stroke-audit.ts`로 원본 해시 확인과 전체 후보 점검을 재현할 수 있다. `--json`은 글자별 상태를 출력한다. 후보 누락 5,456자, 획수 불일치 15자, 공식 대조가 필요한 후보 507자로 분류한다. 기존 검증된 자체 경로가 있는 글자도 AnimCJK 후보까지 검증된 것으로 간주하지 않는다. 원문 대조 없이 재생을 활성화하지 않는 테스트를 추가했다.
