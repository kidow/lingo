...
현재 총 491자(자체 중심선 18자 + AnimCJK Ko 기반 470자 + Ja 3자)의 재생을 제공한다. 5급까지 500자 중 性·者·都를 보정한 488자와, 별도 공식 도해의 回·瓦·臼 3자다. 나머지 5급 범위 12자는 보류한다. [점 보정 기록](hanja-stroke-review-dots.md)에 출처와 보정 방법을 남긴다. 기존 Web Animations 재생기와 Arphic Public License를 유지한다.

`node scripts/hanja-stroke-audit.ts`는 고정된 Ko·Ja 원본 해시, 변환 후 좌표, 性의 보정 순서, 者·都의 점 삽입·원본 획 대응·경로 해시와 전체 도해 위치를 검증한다. `--json`은 글자별 상태를 출력한다. Ko 후보는 누락 5,456자, 획수 불일치 15자, 미승인 39자, 원본 획수까지 맞는 등록 468자이며 Ja 등록 3자는 별도 집계한다. 원본 획수 불일치 15자 중 者·都는 보정 후 재생된다. 따라서 Ko 기반 등록 경로는 470자다. 기존 자체 경로 18자의 다른 후보까지 승인하지 않는다. 읽기 전용 `scripts/hanja-stroke-review.py`의 `/reviewed`에서 공식 도해와 실제 배포 경로를 비교할 수 있다.
