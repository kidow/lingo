...
확인일: 2026-09-07. 웹의 제작자 문서·공식 시험기관 자료와 공개 데이터 파일을 확인했다. 앱 재생 데이터나 코드는 변경하지 않았다.

## 2026-09-07 적용

기존 Web Animations 재생기를 유지하고 Motion 의존성은 추가하지 않았다. 8급 50자의 공식 누적 도해를 확인해 기존 자체 중심선 18자에 AnimCJK 한국어 중심선 31자를 추가했다. 총 49자가 재생되며 萬은 공식 13획·후보 12획의 분할 차이로 보류한다. 원문 위치와 대조 범위는 [필순 검증 목록](hanja-stroke-sources.md#현재-적용한-49자)에 기록한다. 추가 데이터와 Arphic Public License·변환 내역은 `public/hanja-strokes/`에서 제공한다.

`node scripts/hanja-stroke-audit.ts`로 원본 해시 확인과 전체 후보 점검을 재현할 수 있다. `--json`은 글자별 상태를 출력한다. 후보 누락 5,456자, 획수 불일치 15자, 공식 대조가 필요한 후보 476자, 대조 후 등록한 후보 31자로 분류한다. 기존 검증된 자체 경로 18자의 AnimCJK 후보까지 검증된 것으로 간주하지 않는다. 원문 대조 없이 재생을 활성화하지 않으며, 가져온 데이터의 해시·좌표가 달라지면 오류를 내는 테스트가 있다. `scripts/hanja-stroke-review.py`는 원문과 후보의 누적 단계를 나란히 보여주는 읽기 전용 대조 도구다.
