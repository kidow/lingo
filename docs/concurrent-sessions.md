...
**8 · 넘긴 것은 회차 앞에서 줍는다.** 내가 만들지 않은 개념에서 문제를 찾으면
고치지 말고 [docs](.)에 적어 넘긴다. 그런데 **그 파일이 언제 비는지는 아무도
알려 주지 않는다.** 목록에 적어 둘 수도 없다 — 반나절이면 뒤집힌다.

```bash
pnpm pending --free     # 지금 열려 있는 파일의 일감만
```

2026-09-09에 `compass`를 세 번 시도해 세 번 다 막혔다. 이 명령을 만든 날
`travel`과 `idea`가 동시에 빈 틈이 잡혀 그 자리에서 끝냈고, 이틀에 걸쳐
아홉을 그렇게 줍었다. 0.3초 걸린다 — [AGENTS.md](../AGENTS.md)의 작업 순서에
0번으로 넣어 두었다.

**두 파일이 다 열릴 필요는 없다.** 닮은 쌍이 걸렸을 때 고칠 쪽은 **빌린
쪽 하나**다. `cracked`(금 간)가 `plate`(접시)를 그린 자리에서 `food`는
만져지는 중이었지만 `quality`만 열려 있으면 됐다 — 접시는 손대지 않는다.
쌍을 통째로 «막혔다»고 보면 기다리는 시간이 두 배가 된다.

## 규칙을 도구가 지킨다

규칙 여덟은 사람이 지켜야 하는 것이고, 사람은 잊는다. 2026-09-09에 «그림만
바꾸는 일이니 겹치지 않는다»고 여겨 여섯을 고쳤다가 임자의 미커밋 프롬프트를
잃었다. 그날 셋을 도구로 옮겼다.

| 도구 | 무엇을 한다 |
| --- | --- |
| `pnpm dup <slug…>` | 걸린 개념 줄에 `⟨손대는 중⟩`을 붙인다 |
...
그 조건은 자주 깨진다. 지금은 비어 있는 파일 하나를 고르고, 하나도 없으면
건너뛴다.

**막이를 손볼 때는 둘 다 돌린다.** 어느 검사가 무엇을 잡고 무엇을 놓치는지는
[nets.md](nets.md)에 모아 두었다.
...
**남의 개념 그림을 «그림만»이라고 부르지 않는다.** 없는 그림을 채우는 것은
`.webp`만 늘어 겹치지 않지만, **있는 그림을 바꾸려면 `image_prompt`를 고쳐야
한다** — 그건 남의 파일 한 줄을 바꾸는 일이다.

2026-09-09에 이걸 헷갈려 사고를 냈다. 닮은 그림 목록을 임자에게 넘겨 두고도
«그림만 바꾸는 일»이라 여겨 여섯을 직접 고쳤는데, 그 사이 임자 세션이 같은
개념을 손보는 중이었다(`idle`의 그림을 지우고 `monthly`를 다시 그리고 있었다).
내 편집이 **아직 커밋되지 않은 프롬프트를 덮었고**, 되돌릴 때는 HEAD 값밖에
없어 그 세션의 작업이 사라졌다. 로그에 남은 것은 앞 48자뿐이었다.

작업 트리에서 남의 미커밋 수정을 되살릴 방법은 없다. **덮기 전에
`git diff --name-only HEAD -- content`로 그 파일이 지금 만져지고 있는지 본다.**

**검사는 남의 미커밋 콘텐츠로 시끄러워지지 않는다.** `lib/audio-have.ts`는
커밋되는 생성물이라 HEAD 콘텐츠로 굽는데, 검사는 작업 트리를 본다 — 다른
세션이 개념을 넣어 두고 아직 커밋하지 않았으면 늘 어긋난 것으로 보인다.
그래서 어긋난 열쇠가 **전부 아직 커밋되지 않은 파일에서 왔으면** 경고 대신
기록만 남긴다(`git diff --name-only HEAD -- content`로 가린다).
