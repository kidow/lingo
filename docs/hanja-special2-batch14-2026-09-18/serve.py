    ...
    def do_GET(self):
        try:
            ...
            if self.path.startswith('/metadata'):
                ...
                content = f'<h1>{index+1}/{len(DATA["entries"])} · {glyph} · {len(paths)} strokes · {entry["corpus"]}</h1>'
...
if __name__ == '__main__':
    server = ThreadingHTTPServer(('127.0.0.1', 51757), Handler)
    print('Special2 batch11 review http://127.0.0.1:51757', flush=True)
