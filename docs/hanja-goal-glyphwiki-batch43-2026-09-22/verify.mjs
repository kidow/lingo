...
assert.equal(hanjaStrokeData({glyph:'芭',strokes:8}),null)
assert.equal(finding.approved,false);assert.equal(finding.runtimeAdded,0)
...
if(process.argv.includes('--engine')){
 ...
 const output=vm.runInContext(`const k=new Kage();for(const r of Object.values(records))k.kBuhin.push(r.name,r.data);const groups=[];const originalDraw=dfDrawFont;dfDrawFont=function(...args){const polygons=args[1],start=polygons.array.length;originalDraw(...args);groups.push({raw:args.slice(2),polygons:polygons.array.slice(start).map(p=>p.array)});};const polys=new Polygons();k.makeGlyph(polys,root);JSON.stringify({groups,defaults:{kMage:k.kMage,kWidth:k.kWidth,kMinWidthT:k.kMinWidthT,kAdjustMageStep:k.kAdjustMageStep}})`,ctx,{timeout:2000})
 const actual=JSON.parse(output);assert.deepEqual(actual.groups,proof.groups);assert.deepEqual(actual.defaults,proof.defaults);engineChecked=true
}
console.log(JSON.stringify({passed:true,sourceVersions,engineChecked,rawGroups:9,strokeGroups:8,lastStrokePolygons:7,runtimeAdded:0,privateMediaSaved:false}))
