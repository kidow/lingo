...
assert.equal(source.root,'u962d');assert.deepEqual(source.missing,[])
...
assert.deepEqual(trace[4][0].args.slice(4,6),[99.1,79])
assert.deepEqual(trace[5][0].args.slice(0,2),[80.2,80])
assert.notDeepEqual(trace[4][0].args.slice(4,6),trace[5][0].args.slice(0,2))
assert.equal(hanjaStrokeData({glyph:'阭',strokes:7}),null)
...
console.log(JSON.stringify({passed:true,sourceVersions,engineChecked,rawGroups:9,reviewedDomesticStrokes:7,stroke4Discontinuous:true,runtimeAdded:0,privateMediaSaved:false}))
