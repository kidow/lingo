...
추상어로 가면 카드 4종이 전제하는 이미지가 성립하지 않는다.
...
추상어처럼 그릴 수 없는 개념은 **콘텐츠에 넣지 않는다.** 카드 4종이 전부 이미지를 전제하기 때문이다.
...
목록에 넣기 전에 거른다 — **그릴 수 있는가.** 카드 4종이 전부 이미지를 전제한다.
...
**발음은 없어도 된다.** 이미지와 다르다 — 카드 4종이 전부 이미지를 전제하는 것과 달리
...
3. **카드 4종** — 소개 / 재인 / 문맥 / 단서 회상 렌더러와 채점. 손으로 쓴 개념 2~3개로 굴린다
