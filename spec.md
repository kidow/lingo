...
- 네이티브 `<select>`를 투명하게 덮어 쓴다. 모바일에서는 OS 피커가 뜨고 키보드
  조작과 접근성을 브라우저가 준다 — 항목이 둘뿐인데 직접 만들 이유가 없다.
...
- 개념마다 이미지 · 읽기 · 뜻 · 품사 · 예문 · 발음의 유무를 한 줄로 늘어놓는다.
...
런타임 의존성이 `next` · `react` · `ts-fsrs` · `lucide-react` 넷뿐이다.
환경변수도, API 키도 없다.
