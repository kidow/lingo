...
- 뷰포트가 `481px × 800px` 이상일 때만 적용한다. 그 이하는 모바일 배치를 유지한다.
- 이미지와 본문을 `18px` 간격의 한 학습 묶음으로 보고 화면 높이 `46%`에 광학 정렬한다.
- 카드 종류와 관계없이 이미지 기준선과 `196px` 본문 슬롯을 고정한다.
- 카드 레일 `480px`, 이미지 최대 너비 `440px`는 늘리지 않는다.
  피드 속도는 전적으로 손에 있다.
