...
| JLPT | 일본어 | `N5`~`N1`. 트랙을 쪼개지 않고 카드에 표시만 한다 (§5) |

**트랙은 언어를 대체하지 않고 위에 얹힌다.** 발음은 시험이 아니라 언어의
것이라(`public/audio/{language}/`) 같은 단어를 다른 트랙이 써도 mp3를 공유한다.
나중에 JLPT를 N5·N4로 쪼개거나 같은 영어에 다른 트랙을 더해도 이 구조가 버틴다.
...
`content/toeic.json` · `content/jlpt.json`. 개념이 어느 트랙인지를 필드로 두지 않고
**파일 경계로** 정한다 — 한 개념이 두 트랙에 걸치는 일이 없고, 트랙을 늘리는 것이
파일을 늘리는 것과 같아진다. 로더가 읽으면서 `track`을 붙인다 (`lib/content.ts`).
...
- 레벨이 있으면 **시트 좌하단에 한 줄**로 적는다(`N5`). 배지가 아니라 `--sub` 색의
...
1. 트랙 파일(`content/{track}.json`)에 개념 블록을 쓴다 — `slug`, `meaning_ko`, `category`, `image_prompt`, 그 트랙 언어의 단어
2. `pnpm check` — 검증 (아래)
...
| 데이터 | `content/{track}.json` | 빌드 시점 import. 파일이 곧 트랙이다 (§4). DB 없음 |
