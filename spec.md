...
| 신호 | 기준 | 왜 |
|---|---|---|
| 저장소가 무거워짐 | **`.git` 500MB 초과** (지금 219MB) | GitHub 권장선은 1GB. 그 절반에서 움직여 여유를 남긴다. clone은 증분이 아니라 전부 받는다 |
| 발음 전량 재생성 | 목소리·포맷을 바꾸기로 **정한 시점** | 15,000개를 다시 뽑으면 히스토리에 사본이 한 벌 더 쌓여 `.git`이 두 배가 된다. **옮기고 나서 재생성한다 — 순서가 반대면 늦다** |
| 호스팅 이전 | 배포당 **파일 개수 한도가 있는 곳**으로 갈 때 | Cloudflare Pages는 20,000개. 지금 이미지까지 17,000개가 넘는다 |

배치 30개당 약 2.7MB가 붙는다(mp3 210개 + 이미지 30장). 개념을 1,000개 더
넣어도 300MB대라 첫 신호까지는 여유가 있다.

**푸시는 증분이라 배치마다 2.7MB만 올라간다.** 무거워지는 것은 clone과 CI다 —
...
**개념 수로는 답이 안 나온다.** 2,334개를 넣고도 HSK 1~3급의 5분의 1밖에
