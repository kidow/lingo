...
- 소개 카드와 답한 뒤에는 스와이프 안내 문구를 표시하지 않는다.
...
- 안내 카드는 소개 카드와 같은 골격이다 — 이미지 타일(폰 아이콘) → 제목 → 설명.
  하단 Cue는 없고, 화면 세로 중앙에 놓는다. 하단 여백을 남기면
  "다음 장이 있다"는 신호(brand-spec)가 되는데 안내 화면에는 다음 장이 없다.
