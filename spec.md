...
| TORFL | 러시아어 | **비어 있다** — ТРКИ는 어휘 목록을 기계가 읽을 수 있게 내놓지 않는다 |

일곱 트랙 가운데 **여섯이 콘텐츠 전량을 덮는다.** TOEIC만 TSL 필터를 한 겹 더
거치므로 분모가 작다 (§4).

레벨이 채워지는 정도는 트랙마다 다르다 — JLPT 52% · HSK 50% · TELC 27% ·
TOEIC 25% · DELE·DELF·TORFL 0%. **UI는 트랙을 가리지 않는다**(같은 코드가
`levelOf`를 읽는다). 차이는 데이터이고, 채울 수 없는 것은 비운다.

스페인어·프랑스어에는 한때 초기 51개 개념에만 손으로 넣은 `A1`이 남아 있었다.
2%만 등급이 보이면 나머지가 "등급 없는 낱말"처럼 읽히므로 걷어냈다 —
**출처에서 온 것만 붙인다는 규칙이 손으로 넣은 값에도 적용된다.**

**TORFL은 레벨 줄이 비어 있다.** 없는 등급을 지어내지 않으므로(§5) 카드 좌하단은
자리째 빠진다. TSL이 붙기 전의 TOEIC과 같은 상태다 — 목록을 찾으면 그때 채운다.
...
지어내지 않는다 — 없으면 그 카드에 레벨 줄이 안 나온다.
