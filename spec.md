...
**발음은 저장소에 그대로 둔다.** 개념당 7개(언어 수)라 지금 20,881개 · 260MB지만,
...
| 발음 전량 재생성 | 목소리·포맷을 바꾸기로 **정한 시점** | 20,881개를 다시 뽑으면 히스토리에 사본이 한 벌 더 쌓여 `.git`이 두 배가 된다. **옮기고 나서 재생성한다 — 순서가 반대면 늦다** |
| 호스팅 이전 | 배포당 **파일 개수 한도가 있는 곳**으로 갈 때 | Cloudflare Pages는 20,000개. 지금 발음 20,881개에 이미지 2,983장이라 **이미 넘었다** — Vercel에 남는 이유가 이것이다 |
...
**개념 수로는 답이 안 나온다.** 2,983개를 넣고도 HSK 1~3급의 5분의 1밖에
...
| 예문 여러 개 | 지금은 단어당 한 줄(`Word.example`)이다. 늘리려면 `sentences` 배열로 승격 | 언어별이므로 개념이 아니라 단어에 붙는다. **문맥 카드가 생기면서 값이 올라갔다** — 예문이 하나뿐이면 그 칸을 다시 만날 때 같은 문장이 나와, 문장째 외우면 훈련이 무력해진다 |
