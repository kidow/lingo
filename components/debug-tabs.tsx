 ...
 *
 * 넷은 세는 단위가 다르다. 단어 표는 개념 × 트랙이 한 줄, 상식 표는 노트
 * 하나가 한 줄, 의심 문항 표는 문항 하나가 한 줄, 한능검 표는 급수 하나가
 * 한 줄이다. 한 화면에 겹쳐 놓으면 어느 숫자가 무엇의 숫자인지 흐려져서
 * 자리를 나눈다 — 피드의 덱 탭과 같은 이유다 (lib/deck.ts).
 *
 * 한능검이 단어 표에 못 들어가는 이유는 단위뿐이 아니다. 검사할 것도 다르다 —
 * 그림·발음·예문·등급 네 칸이 한자에는 아예 없다 (components/debug-hanja.tsx).
 *
