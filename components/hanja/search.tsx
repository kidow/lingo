...
export function HanjaSearch({ characters }: { characters: HanjaCharacter[] }) {
  ...
  const term = query.replace(/\s/g, '').normalize('NFC')
  const matches = characters.filter((c) => [c.glyph, hunEum(c), c.example.word, c.example.reading, c.example.meaning]
    .some((value) => value.replace(/\s/g, '').includes(term)))
  return (
    <Drawer.Root handleOnly onOpenChange={(open) => { if (!open) { setSelected(null); setQuery('') } }}>
      ...
      <Drawer.Portal>
        ...
        <Drawer.Content className="fixed inset-x-0 bottom-0 z-50 mx-auto flex h-[85dvh] w-full max-w-[480px] flex-col rounded-t-card bg-bg outline-none" lang="ko">
          ...
          {selected ? (
            ...
            <>
              <input aria-label="한자 검색" placeholder="山, 메 산, 산림" value={query} onChange={(event) => setQuery(event.target.value)} className="mx-5 mb-3 min-h-12 rounded-ctrl border border-line bg-surface px-4" />
              <div className="min-h-0 overflow-y-auto px-5 pb-8">
                {matches.map((character) => (
                  <button key={character.id} type="button" onClick={() => setSelected(character)} className="flex w-full items-center gap-4 border-b border-line py-4 text-left">
                    ...
                    <span>{hunEum(character)}<span className="mt-1 block text-sm text-sub">{character.readingGrade}</span></span>
                  </button>
                ))}
