...
**수치가 진실이고 이름은 안내다.** 콘솔은 수치 대신 등급 이름만 보여주는데, 그
이름이 어떤 값에 대응하는지는 문서화되어 있지 않다. 위 이름은 표준 오디오 용어에서
짐작한 것이다 — Telephony 8k · Wideband 16k · **Broadcast 24k** · High quality 32k ·
CD quality 44.1k · Studio 48k. 비트레이트도 Low → Max 다섯 단계 중 가운데인 `High`를
64kbps로 본다.

그래서 **받은 파일은 반드시 `ffprobe`로 확인한다**(아래 검수 체크리스트). 값이 다르면
이름을 다시 고르지 말고 `ffmpeg`으로 재인코딩하면 된다. 이름이 무엇이든 레포에 들어가는
파일은 24000 Hz · 64 kbps · 모노다.
...
콘솔에만 있는 나머지 스위치는 이렇게 둔다.

| 스위치 | 값 | 왜 |
|---|---|---|
| Streaming optimization | **Quality** | 스트리밍을 하지 않는다. 파일 하나를 받아 레포에 커밋하고 끝이라 첫 소리까지의 지연이 아무 의미가 없다. 지연을 내주고 음질을 받는다 |
| Text normalization | **끈다** | "숫자·날짜·약어를 말로 풀어 읽는" 기능이다. 읽히는 것은 `ねこ` 같은 **단어 하나**라 풀어 쓸 것이 없고, 켜 두면 입력과 들리는 소리가 달라질 여지만 생긴다. 들리는 소리와 정답은 어긋나면 안 된다 |
| Timestamps | **끈다** | 구간 타임스탬프를 쓸 곳이 없다. 응답만 무거워진다 |

숫자를 읽히는 개념(`いち`, `にじ` 같은 것)이 콘텐츠에 들어오면 그때 Text normalization을
다시 본다. 그때도 원칙은 같다 — **`reading` 값이 그대로 들려야 한다.**
