# IMAGE_STYLE

단어 이미지의 스타일 규칙. **이 파일이 이미지 스타일의 단일 진실 소스**다.

이미지는 장식이 아니라 학습 요소다. 목표는 예쁜 그림이 아니라
**80×80 픽셀로 줄여도 무엇인지 즉시 알아볼 수 있는 그림**이다.

프롬프트는 두 조각으로 나뉜다.

- **STYLE_PROMPT** — 모든 개념에 공통. 이 파일에만 있다
- **image_prompt** — 개념별 내용. `content/*.json`의 각 개념에 있다

이미지는 **개념**에 속한다. 언어에 속하지 않는다. 바나나 그림 한 장을 모든 언어가 공유한다.

`pnpm prompts`가 `STYLE_PROMPT + image_prompt`를 합쳐 최종 문구를 출력한다.
스타일을 바꾸려면 이 파일만 고치면 된다.

---

## STYLE_PROMPT

ImageGen에 그대로 들어가는 영어 원문이다. 수정 시 아래 블록만 교체한다.

```text
Flat editorial illustration.

Square 1:1 canvas. Plain warm off-white background (#FAF6EF) — no pattern, no
gradient, no vignette, no frame, no border.

One subject, centered, occupying 65-75% of the frame. Generous even margins. The
subject may be a single object, or a small scene of at most three related elements
that read as one thing.

Limited palette of 4-6 muted earthy colors. Low to medium detail built from simple
geometric shapes. The silhouette must stay clear and recognizable when the image is
scaled down to 80x80 pixels.

No outlines, except a minimal one only where two similar tones meet. One small soft
contact shadow directly beneath the subject; no cast shadows, no ambient occlusion,
no texture noise, no grain.

Even diffuse lighting with no visible light source.

No text, no letters, no numbers, no logos, no watermark, no speech bubbles, no UI
elements, no captions.
```

---

## 개념 유형별 작성법

`image_prompt`에는 **내용만** 쓴다. 스타일 단어(flat, minimal, illustration, pastel 등)를
넣으면 STYLE_PROMPT와 충돌하므로 쓰지 않는다.

### 「글자 없음」이 막는 것과 안 막는 것

STYLE_PROMPT의 마지막 줄은 **읽히는 것**을 막는다 — text · letters · numbers ·
logos · watermark · speech bubbles · UI elements · captions. 그림 기호는 그
목록에 없다. 화살표·체크 표시·과녁·신호 표시는 **막지 않는다.**

전수로 세어 보면 이 갈래가 갈린다.

| | 개수 | 판정 |
| --- | ---: | --- |
| 프롬프트가 화살표·기호를 부른다 | 88 | 규격 위반이 아니다 |
| 프롬프트가 금지 낱말을 그대로 부른다 | 7 | 아래 규칙으로 다 통과한다 |
| 실제 위반 | **0** | |

**금지 항목이 개념 자체일 때는 비워서 그린다.** `subtitles`(자막)·
`dialogue`(대화)·`text-message`(문자)·`language`(언어)는 말풍선이나 자막 띠가
없으면 그릴 수가 없다. 그래서 **틀만 그리고 안을 비운다** — 말풍선은 빈 채로,
자막 띠는 물결 자국으로, 간판은 무늬 없는 판으로 둔다. 읽히지 않으면 규격을
지킨 것이다.

```
text-message → a phone screen showing a single empty speech bubble
dialogue     → two speech bubbles facing each other with plain line marks inside,
                no readable letters
subtitles    → a television screen … with a light caption bar of blank wavy marks
```

**이 자리는 검사로 옮기지 않는다.** 후보 88개 중 참이 0개라 경고가 전부
헛것이 된다 — 시트 검사가 헛경고 때문에 스스로 꺼져 있던 사고와 같은 꼴이다
(scripts/genimg.sh). 프롬프트를 쓸 때 위 세 줄을 지키는 것으로 갈음한다.

#### 예외 하나 — 이름이 곧 뜻인 낱말은 **한국어 표기**를 그림에 넣는다

달 이름 열둘이 이 자리다. 1월과 2월의 차이는 **열둘 가운데 몇째냐**뿐이고
그 밖에 그릴 것이 없다. 세 가지를 차례로 시험해 셋 다 졌다.

| 장치 | 무엇이 안 됐나 |
| --- | --- |
| 열두 쪽 고리에서 n번째를 칠함 | 모델이 열둘을 못 센다 — 칸이 열에서 열둘로 흔들린다 |
| 3×4 격자에서 n번째 알을 빨갛게 | 세기는 맞는데 **「달력」이 아니라 「점 열두 개」로 읽힌다** |
| 달력 + 오른쪽 색인 탭 열둘 중 n번째 빨강 | 달력으로는 읽히는데 80×80에서 탭이 안 보인다 |

**달력 머리띠에 「1월」이라고 한국어로 적으니 한 번에 풀렸다.** 글자가 안
깨지고(모델이 한글을 정확히 쓴다) 80×80에서 이웃한 달이 완전히 갈린다.

근거는 이 앱의 모양이다. **가르치는 말은 일곱이지만 배우는 이의 모어는
하나(한국어)다.** 카드는 「그림 → 외국어 낱말」을 묻는다. 그림이 뜻을 나르는
것이 본래 일이고, 뜻을 한국어로 적는 것은 답을 흘리는 것이 아니다.

**못은 두 군데 박는다.**

- **외국어 표기는 절대 안 넣는다.** `January`·`янва́рь`를 적으면 그건 답이다.
- **이 예외는 이름이 곧 뜻인 낱말에만 쓴다.** 그림으로 갈 길이 **있는데도**
  글자로 가면 규격이 무너진다. 수(13~19)는 세는 물건으로 이미 풀리고
  (`eleven`·`twenty`·`hundred`), 요일 일곱도 「일곱 칸 중 n번째」로 선다.
  **열둘부터 세기가 무너진다** — 그 자리에서만 글자를 쓴다.

**두 번째 자리 — 국적과 언어.** 2026-09-17에 TORFL A1의 국적말 열여덟을
넣으며 같은 벽에 부딪혔다. 「러시아의」와 「프랑스의」를 가를 그림이 상투
말고는 없다 — 사모바르·베레모·이층버스로 가면 **그 물건의 그림이지 그
나라의 그림이 아니고**, 여섯 나라를 그렇게 세우면 여섯 다 틀린다.

달과 같은 잣대가 그대로 선다. 나라 이름은 **이름이 곧 뜻**이고, 그림으로
갈 길이 아예 없다. 그래서 천에 「러시아」「영국 사람」「중국 말」이라고
한국어로 적었다. 외국어 표기를 안 넣는 못은 여기서 더 세다 — `Russian`을
적으면 영어 답이 그대로 드러나고, `русский`를 적으면 러시아어 답이 드러난다.

한 벌이 열여덟이라 **꼴과 빛깔 두 축으로 갈랐다.**

| 축 | 어떻게 |
| --- | --- |
| 꼴(품사) | 세모 기(형용사) · 봉랍 도장(나라의) · 네모 기(사람 남) · 물결진 기(사람 여) · 두루마리(~말로) |
| 빛깔(나라) | 여섯을 멀리 떼어 잡는다 |

**빛깔은 처음부터 떼어 잡는다.** 열두 달에서 8월(terracotta orange)과
9월(clay red)이 너무 가까워 `pnpm twins`에 구조 31·색 0.15로 걸렸다.
한 벌을 지을 때 이웃한 빛깔을 쓰면 **나중에 반드시 걸린다** — 여섯 나라는
dusty blue grey · deep navy · terracotta orange · warm ochre · clay red ·
sage green으로 처음부터 벌려 두었다.

**한 벌은 소품을 나눠 쓴다.** `pnpm props`가 기·두루마리·한글을 열여덟에서
한꺼번에 잡아내는데 이건 고칠 자리가 아니다. 열두 달이 달력 한 장을 나눠
쓴 것과 같다 — **한 벌로 묶인 것은 틀을 나눠 쓰는 편이 읽는 쪽에 낫다.**
소품 겹침을 피하려고 열여덟을 제각각으로 세우면 한 벌로 안 읽힌다.

**세 번째 자리 — 주의(主義) 이름.** 2026-09-18에 TORFL B2의 `-изм`/`-ист`
무리 열일곱을 넣으며 또 같은 벽이었다. 「민족주의」와 「인종주의」를 가를
그림이 없다 — 깃발로 가면 나라 그림이 되고, 주먹·사슬·저울로 가면 그
물건의 그림이지 그 주의의 그림이 아니다. 게다가 **주의·사람·형용사가 한
뿌리에서 셋씩 갈라져** (`национализм`·`националист`·`националистический`)
물건 하나로는 셋을 못 가른다.

나라 이름과 잣대가 같다. 그래서 한국어로 「민족주의」「민족주의자」
「민족주의의」라고 적고, **꼴로 품사를, 빛깔로 뿌리를 갈랐다.**

| 축 | 어떻게 |
| --- | --- |
| 꼴(품사) | 둥근 헝겊 배지(주의) · 네모 나무 명패(사람) · 좁은 띠(~의) |
| 빛깔(뿌리) | 자본 moss green · 공산 brick red · 사회 mustard yellow · 민족 slate blue · 인종 charcoal grey · 애국 burnt sienna · 비관 dusty violet · 낙관 pale apricot · 이상 teal · 개인 sand beige · 이기 rust brown |

한 뿌리가 빛깔을 나눠 쓰되 **꼴이 다르므로** `pnpm twins`의 구조 축에서
갈린다 — 배지와 띠는 같은 빛깔이어도 구조가 멀다. 나라 기가 그랬던 것과
같은 얼개다.

**사물 한 개로 그려야 하는 것은 아니다.** 단어가 요구하면 장면으로 간다 —
`cafeteria`를 쟁반 하나로 그리면 쟁반이 된다. 대신 공통 규격(배경·팔레트·글자
없음·80×80 판독)은 어떤 유형이든 그대로다. 유형별로 달라지는 것은 **무엇을
프레임에 담느냐**뿐이다.

### 구체 명사 (사물 · 동물 · 음식)

단일 객체. 정면 또는 3/4 측면, 중립 자세. 배경 소품을 넣지 않는다.

```
apple     → a single red apple with one green leaf on the stem, seen from the side
cat       → one sitting cat facing forward, tail curled around its front paws
umbrella  → one open umbrella seen from the side, handle pointing down
```

### 동사

동작 한 컷. 인물은 최대한 단순화하고 **얼굴 이목구비는 넣지 않거나 점 두 개까지만**.
동작이 실루엣만으로 읽혀야 한다.

```
eat   → one person lifting food to their mouth with chopsticks, seen from the side
run   → one person mid-stride running to the right, both feet off the ground
sleep → one person lying on their side asleep under a blanket, seen from the side
```

### 형용사 · 상태

셋 중 하나를 고른다.

1. **성질형** — 물체 자체가 그 뜻인 것. 제일 좋다
2. **상태형** — 같은 물체의 상태만 다르게. 반의어 쌍에 잘 맞는다
3. **대비형** — 둘을 나란히. 마지막 수단이다

```
hot   → one cup with three wavy heat lines rising from it        (성질형)
slow  → one snail crawling to the right leaving a short trail    (성질형)
empty → one drinking glass with nothing inside                   (상태형)
full  → one drinking glass filled to the brim with orange juice  (상태형)
```

**대비형은 주인공을 키운다.** 크기가 다른 물체 둘을 그냥 나란히 두면 `big`과
`small`이 같은 그림이 된다 — 학습자가 보는 것은 "크기가 다른 물체 둘"이지
어느 쪽을 가리키는지가 아니다. 실제로 그렇게 나왔다.

말하려는 쪽을 **중앙에 크게** 두고, 비교 대상은 **가장자리에 작게 또는 잘라서**
둔다. 시선이 먼저 닿는 것이 정답이다.

```
big   → one large suitcase filling most of the frame, with a much smaller
        suitcase of the same shape at the lower right corner
small → one small teapot centered, with a much larger teapot of the same shape
        cropped at the left edge behind it
```

### 색 이름 — 팔레트 규칙을 **그 색에만** 푼다

`red`·`blue` 같은 색 이름은 색 자체가 뜻이다. 공통 팔레트(무채도 4~6색)를
그대로 적용하면 빨강과 주황이 같은 흙색으로 뭉개져 카드가 구별되지 않는다.

그래서 색 이름 개념에서는 **피사체 하나만 그 색을 또렷하게** 칠하고, 배경과
나머지 요소는 규격대로 둔다. 형태는 색을 담는 그릇일 뿐이므로 짜 놓은 물감
덩어리처럼 단순한 것으로 통일한다 — 형태가 다르면 학습자가 형태를 외운다.

```
red  → one open paint tube with a thick blob of clear red paint squeezed out
blue → one open paint tube with a thick blob of clear blue paint squeezed out
```

### 감정 · 상태 — 얼굴이 아니라 **몸과 상황**으로 그린다

스타일이 이목구비를 그리지 않으므로 표정으로는 감정을 전할 수 없다. 대신 세 가지를 쓴다.

1. **자세** — 처진 어깨, 든 팔, 웅크린 등
2. **거리와 위치** — 넓은 화면에 홀로 앉은 사람(외로움), 문 뒤에 반쯤 숨은 사람(부끄러움)
3. **곁의 사물 하나** — 텅 빈 접시(배고픔), 쌓인 서류(바쁨), 비어서 뒤집힌 지갑(가난)

```
tired  → one figure slumped over a desk with their head resting on both arms
lonely → one small figure sitting alone on a bench with wide empty space around
busy   → one figure carrying a tall stack of folders with a phone in the other hand
```

기호는 최소한만 쓴다. 머리 위 먹구름·김 나는 선 정도는 허용하되, 말풍선과
글자·기호는 여전히 금지다(체크표·물음표도 글자다).

**사물이 얼굴을 품는 자리가 있다.** 프롬프트에 사람이 하나도 없어도 신분증·
여권·사진틀을 그리라고 하면 모델이 그 안에 초상을 넣는다 — 28회차의
`id-for-claim`이 그랬다. 인물 규칙은 `figure`·`person` 같은 **사람 낱말만**
보므로 이 자리를 못 잡는다.

`pnpm check`가 신분증·여권류를 보면 알려 준다. **막지는 않는다** — 닫힌 여권,
이름 없는 명찰처럼 초상이 안 들어가는 자리가 더 많고, 들어갔는지는 결국 시트를
봐야 안다. 넣기 싫으면 `blank front`나 `no portrait`를 적는다.

넣어도 되는 초상이 하나 있다. **이목구비 없는 실루엣**은 스타일 안이다 —
`shareholder`의 명찰과 `residence-card`의 카드가 그렇게 그려져 있고, 둘 다
머리와 어깨의 윤곽뿐이다.

### 상황 · 장면

요소는 **최대 3개**. 그 이상은 작게 줄였을 때 죽는다.

```
station → a train stopped at a simple platform with one waiting person, side view
```

**상황 표현은 명사 개념과 같은 사물을 쓴다.** `자막 있나요?`는 자막을, `유통기한이
언제예요?`는 우유갑을 그리게 되는데 그 사물에는 대개 이미 개념이 있다. 사물을
그대로 그리면 그림이 겹치고, 겹치면 `pnpm twins`가 잡거나 4지선다에서 두 카드가
같은 그림으로 나온다.

**후보를 고를 때 `pnpm props`로 먼저 본다** — 그 사물에 개념이 있는지, 다른
그림에 이미 나오는지 찍어 준다(spec.md §7). 프롬프트 초안을 통째로 넣어도 된다.

**사물을 바꾸지 말고 장면을 바꾼다.** 손·창구·사람을 넣어 "그 사물이 놓인 자리"를
그리면 같은 사물로도 그림이 갈린다. 상황 표현은 원래 장면이므로 이 방향이 자연스럽다.

| 이미 있는 명사 | 그대로 그리면 | 장면으로 바꾼다 |
| --- | --- | --- |
| `timetable` 격자 한 장 | 수강 신청이 시간표가 된다 | 사람 둘이 벽의 격자를 짚는다 |
| `report-card` 접힌 카드 | 성적 확인이 성적표가 된다 | 화면의 빈 줄을 손가락이 짚는다 |
| `form` 빈칸 서식 | 계정 만들기가 서식이 된다 | 자판을 치는 두 손, 화면에 빈 프로필 동그라미 |
| `attendance` 책상 배열 | 결석계가 출석이 된다 | 빈 의자 옆 책상에 놓인 접힌 쪽지 |
| `passport` 여권 한 권 | 여권 발급이 여권이 된다 | 손이 접힌 서식을 창구 틈으로 밀어 넣는다 |

**그래도 못 피하면 후보를 뺀다.** 한 회차에서 `자막 있나요?`와 `유통기한이
언제예요?`를 이렇게 뺐다 — 앞은 `subtitles`가 이미 캡션 바가 깔린 화면을 그리고
있었고, 뒤는 `expiry-date`가 slug까지 같았다.

**장면끼리도 겹친다.** 같은 회차에서 문을 세 장 그렸다가 `재배달 신청할게요`가
`회의실을 예약할게요`와 닮아 우편함으로 바꿨다. 사물을 고를 때 그 회차 안에서도
겹치는지 본다. 이 규칙을 세우고 돌린 다음 회차는 열다섯 장에서 되돌린 그림이
0장이었다.

### 장소 · 시설

건물은 정면 한 채로, 실내는 **그 공간을 그 공간으로 만드는 요소 두셋**으로 그린다.
사물 하나로는 못 그리는 단어가 있다 — `cafeteria`를 쟁반 하나로 그리면 쟁반이 된다.

```
supermarket → one shop front with a wide window and two shopping carts outside, seen from the front
cafeteria   → two long tables with bench seats and a counter behind them, seen from the front
aisle       → two tall shelf units facing each other with a walkway between them, seen straight down the walkway
stadium     → one oval stadium bowl with tiered seating and a green field, seen slightly from above
```

요소를 늘릴수록 80×80에서 죽는다. **셋을 넘기지 않는다**는 규칙이 여기서도 그대로다.

### 추상어 — 뜻이 아니라 **예문 장면**을 그린다

`deadline`·`refund`·`authorize`는 그 자체로 그릴 것이 없다. 낱말을 그리려 하지 말고
**예문이 말하는 한 장면**을 그린다. 그림이 뜻을 정의하지 못해도 된다 — 뜻은 카드에
한국어로 이미 적혀 있고, 그림은 그 낱말이 사는 자리를 기억에 걸어 두는 고리다.

```
deadline  → one wall calendar with a single date circled in red and a small hourglass
             in front of it            ("보고서는 마감 기한 전에 내야 합니다")
refund    → one hand passing banknotes back across a counter to another hand holding
             a receipt                 ("가게에서 환불해 주었습니다")
authorize → one hand pressing a round rubber stamp onto a sheet of paper
                                       ("관리자가 결제를 승인할 겁니다")
```

**여섯 언어의 예문이 같은 장면을 말해야 한다.** 그림은 개념당 한 장인데 예문이
언어마다 다른 상황이면 다섯 트랙에서 그림과 문장이 어긋난다.

나머지 규칙은 그대로다 — 요소 셋 이하, 글자·숫자 없음(체크표·엑스표도 글자다),
80×80에서 판독. 장면이 세 요소로 안 담기면 그 낱말은 아직 넣지 않는다.

**장면을 찾는 다섯 수.** "그릴 것이 없다"고 버리기 전에 이 순서로 좁혀 본다.
한 회차에서 후보 마흔넷 가운데 서른둘을 그림이 안 선다고 버렸는데, 아래 수로
되짚으니 그중 여럿이 살았다.

| 수 | 무엇을 그리나 | 예 |
|---|---|---|
| 행위의 순간 | 그 낱말이 일어나는 **찰나** | 부패 → 탁자 밑으로 건네는 봉투 · 나눠 주다 → 한 장이 들린 전단 더미 |
| 결과 상태 | 그 낱말이 지나간 **자리** | 가둠 → 문이 잠긴 빈 새장 · 망가뜨리다 → 물에 젖어 우는 공책 |
| 쓰이는 도구 | 그 낱말을 **일으키는 물건** | 면역 → 백신 병과 반창고 · 점검 → 손전등과 열린 기계 뚜껑 |
| 다루는 방식 | 사람이 그것을 **어떻게 두는가** | 값을 매길 수 없는 → 유리 돔 속 보석 · 사임 → 짐 상자 놓인 빈 책상 |
| 도식 하나 | 뜻을 **모양으로** | 갈등 → 맞부딪어 휘는 화살표 둘 · 늘다·줄다 → 계단꼴로 오르내리는 동전 더미 |

**그래도 못 넣는 것이 있다.** 좁힌 장면이 **이미 있는 다른 개념의 그림과 구별되지
않으면** 넣지 않는다. 비판을 붉게 표시한 원고로 그리면 편집자와 같아지고, 의견을
말풍선으로 그리면 피드백과 같아진다. 아름다움·성격·본질처럼 어떤 장면으로 좁혀도
다른 개념이 되는 낱말은 그대로 둔다 — 목록을 채우자고 그림을 겹치게 하지 않는다.


### 이미지가 하나로 안 그려지면 개념이 둘이다

`水`(찬물)와 `お湯`(더운물)을 한 장에 담으려 하지 말 것. 개념을 `cold-water`와
`hot-water`로 쪼갠다. 이것이 개념을 나누는 실무 기준이다 — 개념은 사전 표제어가 아니라
**시각화 가능한 지시체** 단위다.

---

## 출력 규격

| 항목 | 값 |
|---|---|
| 생성 | PNG 1024×1024 |
| 저장 | WebP 512×512, quality 80 |
| 경로 | `concepts/{concept_slug}.webp` — 언어 구분 없음 |
| 파일명 | seed의 개념 `slug`와 정확히 일치 |

생성한 PNG는 `.images/{slug}.png`에 두고 `pnpm images`를 실행한다.
`.images/`는 gitignore 대상이다. 변환·업로드·`image_path` 갱신은 스크립트가 한다.

---

## 예외 둘 — 규칙이 낱말을 못 그리게 만들 때

**얼굴 부위는 그 부위를 그린다.** `chin`(턱)·`cheek`(볼)·`forehead`(이마)·
`lip`(입술)은 얼굴을 빼면 무엇을 가리키는지 사라진다. 이 낱말들만 옆얼굴에
코·입술선을 허용한다 — 감정을 그리는 것이 아니라 부위를 가리키는 것이다.

**화면을 채우는 것이 곧 주제인 낱말이 있다.** `sky`(하늘)는 배경이 파랗지
않으면 하늘이 아니고, `road`(길)도 그렇다. 배경 규칙(따뜻한 오프화이트)은
사물을 그릴 때의 규칙이지 **장(場) 자체가 주제일 때는 적용하지 않는다.**

전수 감사(2,953장)에서 이 둘 때문에 멀쩡한 그림을 다시 뽑을 뻔했다. 규칙을
지키느라 낱말을 못 알아보게 만들면 규칙이 목적을 배반한 것이다.

---

## 프롬프트를 쓰기 전에 보는 일곱 줄

아래 절들은 실패를 갈래별로 모아 두었고 다 합쳐 백여든 줄이다. **길어서
읽어 두고도 쓸 때 안 떠오른다** — 2026-09-16에 «잉크 밑에 비치는 연필
밑그림»이 화분으로 나오는 실패를 이미 표에 적어 두고도 本意«본디 뜻»에서
그대로 다시 냈다. 그래서 일곱 줄로 줄여 앞에 세운다. 한 줄이라도 걸리면
해당 절을 편다.

1. **그려져야 하는 것이 문장의 주어인가.** «A인데 B인» 꼴이면 B는 사라진다.
   «아래에 비치는», «옆에 걸린», «~은 없는»은 전부 이 꼴이다.
2. **정도를 말로 적지 않았는가.** «가까이», «넓은 여백», «조금 기운»은
   흘러간다. 잘린다고·비는 쪽을 주어로·작은 쪽을 작다고 적는다.
3. **흔한 물건 이름이 다른 뜻으로 먼저 읽히지 않는가.** lens·oil·globe·
   stamp·heart·bill·seal이 그렇다. **집 안 물건 이름은 특히 그렇다** —
   table은 밥상, seat은 의자, measure는 계량컵을 먼저 부른다. 순위표·
   탈것 자리·되를 말하려면 그 낱말을 쓰지 말고 물건을 못 박는다.
4. **견줌이 뜻 자체인가.** 그렇다면 둘을 한 몸으로 매야 한다(도르래의 두 추).
   뜻을 돕기만 하면 버린다.
5. **그릴 것이 눈에 안 보이는 것인가.** 숨김·어긋남·같음·없음·닳음·움직임·
   시름이 다 여기다. 그릴 것 자체는 안 그려진다 — **곁에 남는 자국**으로
   적는다(거미줄·물결 고리·남긴 껍질·뱃머리 물살).
6. **이 낱말이 사람 몸·총·약을 곧바로 부르는가.** 그대로 적으면 거절이 난다.
   말을 다듬지 말고 **그 자리에 선 것**을 인형·옷틀·나무토막으로 갈아 끼운다.
7. **이 소품을 다른 개념이 쓰고 있지 않은가.** `pnpm props --in`을 그림을
   고르기 **전에** 돌린다. 9,300장을 넘긴 뒤로는 몸통보다 **꾸미는 말**이
   먼저 부딪는다.
8. **몸통 낱말을 `pnpm props <낱말>`로 한 번 세었는가.** 배치 안이 아니라
   `content/` 전체를 본다. 아래 「몸통을 먼저 세면 되돌리지 않는다」를 볼 것.

**7번과 8번은 여기 적혀 있는데도 2026-09-21에 두 회차를 그냥 뽑았다.** 이
문서의 머리말이 말하는 병이 그것이다 — 읽어 두고도 쓸 때 안 떠오른다.
되돌린 장으로 열다섯 회차에 다섯·셋을 치렀고, 세고 뽑기 시작한 뒤 네 회차는
0·0·1·0이다.

## 그리지 못하는 지시문 — 배경으로 적은 것은 사라진다

프롬프트에 **주어가 아닌 자리로 적은 것은 그려지지 않는다.** 추상어를 도식으로
그리기 시작하면서 같은 실패를 여섯 번 했다. 전부 "A인데 B인" 꼴로 적었고,
매번 B가 사라졌다.

| 적은 것 | 나온 것 |
|---|---|
| 기울어진 울타리 기둥 옆으로 **몰아치는 눈** | 눈이 하나도 없는 자동차 |
| 벽에 기댄 사다리 — **벽은 창도 문도 없다** | 벽이 없는 사다리 |
| 눈금 **오른쪽 끝**에 붙은 바늘 | 바늘이 왼쪽에 붙은 눈금 |
| 개찰구 **옆에** 걸린 카드 | 개찰구가 화면을 채우고 카드는 구석 |
| 잉크 밑에 **비치는** 연필 밑그림 | 화분, 그리고 다시 뽑으니 머그컵 |
| **삐뚤빼뚤한** 원과 정확한 원을 나란히 | 반듯한 원 둘 |

고치는 법은 하나다. **그려져야 하는 것을 문장의 주어로 올린다.**

  눈이 몰아치다  → "옆으로 몰아치는 흰 눈이 기둥을 지나고, 아래에 눈더미가 쌓인다"
  벽이 막혀 있다 → "벽돌 벽에 기댄 사다리, 꼭대기 칸이 민무늬 벽돌에서 끝난다"
  바늘이 오른쪽 → "바늘이 눈금 오른쪽 끝까지 넘어가 있고 그 끝이 빨갛다"
  카드가 주인공 → "끈에 매달린 커다란 빈 카드가 화면을 채우고, 뒤에 개찰구가 흐리게"

**비교는 특히 안 된다.** "A와 B를 나란히"는 둘 중 하나만 그려지거나 둘이 같아진다.
비교를 버리고 **한쪽만** 그리면 뜻이 선다 — `approximate`는 "삐뚤빼뚤한 원과 정확한
원"을 버리고 "끝이 안 맞는 원 하나"로 적어서야 통했다.

### 크기·비움을 적으면 흘린다

낱말은 제 뜻으로 그려졌는데 **얼마나·어디까지가 안 지켜지는** 자리가 따로 있다.
2026-09-10에 시트가 두 장을 그렇게 잡았다. 둘 다 `twins` 거리는 이미 넉넉히
멀었다 — **기계가 아니라 눈만 잡는 자리다.**

| 적은 것 | 나온 것 | 통한 말 |
|---|---|---|
| `one close view of the front of an eye in cross-section, the clear dome filling most of the frame` | **눈 전체 단면.** 각막은 왼쪽 끝에 작게 | `an extreme close-up cross-section of only the front of an eye … the rest of the eye cut off by the frame edge` |
| `one printed page with a dense block of grey text lines in the middle and a wide untouched border` | 글줄이 종이를 **가득** 채워 여백이 사라졌다 | `one page holding a very small block of grey text lines in the centre, the empty border around it taking up most of the page` |

**둘 다 같은 버릇이다.** 모델은 **물건을 통째로 프레임에 넣으려 하고, 빈 자리를
채우려 한다.** 그래서 «가까이»·«여백»처럼 정도를 말하는 낱말은 흘러간다.

- 잘라내기를 원하면 **잘린다고 적는다** — `extreme close-up` · `only the front
  of` · `cut off by the frame edge`. «filling most of the frame»만으로는 안 된다.
- 비움을 원하면 **비는 쪽을 주어로 삼는다** — `the empty border … taking up most
  of the page`처럼. «wide border»는 크기를 말할 뿐이라 채워진다.
- 무엇을 **작게** 두고 싶으면 그 물건에 `very small`을 붙인다. 큰 쪽을 키우는
  말보다 작은 쪽을 줄이는 말이 잘 듣는다.

### 낱말이 제 뜻으로 안 읽히는 자리

지시문이 사라지는 것과 다른 실패가 하나 더 있다. **낱말은 그려지는데 엉뚱한
물건이 나온다.** 모델이 그 낱말을 다른 뜻으로 먼저 읽어서다.

| 적은 것 | 나온 것 | 고친 말 |
|---|---|---|
| `one lens with a scratch` | 카메 렌즈 (두 번) | `one spectacle frame …` · `one single eyeglass lens standing in a clip stand` |
| `one oil can with a tag` | 주전자 | (다시) `one squat oil bottle` → 올리브유 → `one dipstick with a dark oil mark` |
| `a globe mark printed in its corner` | 야자수와 표지판 | `one certificate sheet with a second sheet clipped behind it` |
| `one booklet with rows of small stamps` | 우표 수집책 · 식물 도감 | `one sheet with three round ink stamps and a small syringe` |
| `one paper strip of a heart trace` | 사랑 하트 리본 | `one strip of graph paper with a jagged line that flattens into a gap` |
| `one paper bill with a red stamp mark` | **지폐** | `one printed notice sheet with a red band across its top edge` |
| `one earth bank with a wet line running out of it` | **지구본** | `one cracked water jar with a wet trail leading away` |
| `one small round lump lifted out whole` | 호박 | `one glass phial holding one small pale round object` |
| `the last three rows of a table shaded darker` | **밥상**을 위에서 본 그림 | `one open trapdoor … with a short ladder going down` |
| `two ruled table rows with a tile at the end of each` | 화분 올린 **탁자** 둘 | `two wooden pegs pushed to the same depth in a slotted board` |
| `a cup of water standing level on a seat` | **밥상 의자**에 물컵 | `one cup of water standing level on a bus seat beside a window` |
| `one empty measure set down hard on a counter` | 투명 플라스틱 **계량컵** | `one empty wooden grain measure set down hard on a plank counter` |
| `one paper window with a small tear` | 봉투 | `one single blossom fallen off at a touch` |
| `three seals of three sizes` | **바다표범 세 마리** | `three brass tallies of three sizes hanging on one hook` |

뒤의 셋은 두 글자 17회차에서 한 회차에 같이 나왔다. `earth`는 흙이 아니라
지구로 읽히고, `lump`는 멍울이 아니라 둥근 열매로 읽히고, `paper window`는
창호지가 아니라 **봉투의 비닐 창**으로 읽힌다. 셋 다 `pnpm props`가
«임자 없음»으로 통과시킨 낱말이다 — props는 그 낱말에 임자가 있는지를 보지
낱말이 다른 뜻으로 읽힐지는 보지 않는다.

**흔한 물건 이름일수록 다른 뜻이 세다.** `lens`는 사진 렌즈, `oil`은 식용유,
`globe`는 여행, `stamp`는 우표, `heart`는 사랑 하트, `bill`은 지폐, `seal`은
바다표범이다. 안경알을
그리려면 `spectacle`이나 `eyeglass`를 붙여야 하고, 엔진 오일은 `dipstick`처럼
**그 맥락에만 있는 물건**으로 바꿔야 한다.

`bill`은 뜻이 **셋**이라 더 험하다 — 지폐·청구서·부리다. 레포 안에서도 셋 다
쓰인다(`utility-bill`이 청구서, `duck`이 부리, `playbill`이 벽보). 청구서를
그리려면 `bill`을 아예 쓰지 말고 `notice sheet`나 `statement`로 적는다.

**검사로 막지 않는다.** 이 여섯 낱말이 프롬프트 92곳에 쓰이는데 거의 다
정당하다 — `lens`는 렌즈 개념이 쓰고, `stamp`는 우표와 도장이 쓰고, `heart`는
심장이 쓴다(2026-09-09 실측: stamp 29 · lens 19 · bill 14 · globe 13 · oil 10 ·
heart 7). 낱말만 보고 경고하면 헛경고가 아흔둘이고 진짜는 회차에 하나다.
**시트를 여는 것이 여전히 그물이다** — 여기 적어 두는 것은 프롬프트를 쓸 때
미리 피하라는 뜻이지, 기계가 잡아 준다는 뜻이 아니다.

같은 실수를 두 번 했다 — 24회차의 `lens-scratched`와 27회차의 `dizzy-glasses`가
똑같이 카메라 렌즈로 나왔다. **시트를 봐야 걸린다**(twins도 해시도 못 잡는다).
29회차의 `late-fee-added`도 그랬다 — 청구서를 그리려고 `paper bill`이라 적었는데
지폐가 나왔고, 구조도 색도 남과 안 닮아 어떤 검사도 울지 않았다.

### 견줄 짝은 한 몸으로 매여 있어야 그려진다

위에서 «비교는 특히 안 된다»고 적어 두었다. 틀린 말은 아닌데 조건이 빠져
있었다. 2026-09-16에 견줌이 꼭 필요한 낱말 다섯을 한꺼번에 만나 보고서야
갈렸다 — **따로 놓인 둘은 «둘»로 읽히고, 한 줄에 매인 둘만 «반대»로 읽힌다.**

| 적은 것 | 나온 것 | 통한 말 |
|---|---|---|
| 反之 `two boots set toe to toe pointing away from each other` | 그냥 **등산화 한 켤레** | `one pulley wheel with a rope over it, the weight on one side risen right to the top and the weight on the other side sunk to the ground` |
| 微微 `one brimming bowl of water tilted just enough that the water touches the rim` | 물 담긴 사발. 기울기가 안 보인다 | `nine identical cups in a row with the last one filled a hair lower than all the rest` |
| 未免 `one hung door a finger too wide for its frame` | 그냥 **문** | `one jar lid a finger too wide for its jar, sitting askew and overhanging the rim all round` |
| 举足轻重 `many weights in one pan and a single tiny weight in the other` | 무더기 쪽이 **내려간** 저울 | `whose pan holding a single tiny weight has sunk right down to the bottom while the pan heaped with many large weights has risen high` |

- **한 몸으로 매어라.** 도르래의 두 추는 하나가 오르면 하나가 내려갈 수밖에
  없다. 신발 두 짝은 서로 아무 상관이 없어 방향을 적어도 눈에 안 남는다.
- **같은 것 여럿에 하나만 어긋나게 두어라.** «살짝»은 사발 하나로는 못
  그린다. 똑같은 잔 아홉 옆에서 마지막 하나가 한 올 낮아야 «살짝»이 된다.
  하나만 그릴 때는 그 어긋남을 **다른 물건에 얹는다** — 문은 문으로 읽히지만
  단지보다 넓어 비뚜로 얹힌 뚜껑은 «좀 과하다»로 읽힌다.
- **두 쪽이 있는 소품은 어느 쪽이 내려가는지 적어라.** 안 적으면 모델이
  물리를 따른다. 저울·시소·두레박이 그렇고, 성어는 대개 **물리를 거스르는
  쪽이 뜻**이다.

앞 절의 `approximate`와 어긋나지 않는다. 그쪽은 견줌을 **버려서** 통했고
이쪽은 견줌이 곧 뜻이라 버릴 수가 없었다. 가를 자리는 하나다 — **견줌이
뜻 자체면 한 몸으로 매고, 뜻을 돕기만 하면 버린다.**

### 소품을 비킬 때는 뜻을 나르지 않는 쪽이 비킨다

`pnpm props`가 배치 안 겹침을 알려 오면 둘 중 하나가 소품을 놓아야 한다.
지금까지는 «어느 쪽이 그 물건을 덜 쓰나»로 골랐는데, 그러다 뜻을 나르던
물건을 뺐다.

军事«군에 관한 일»과 流域«강물이 모이는 땅»이 `map`에서 겹쳤을 때 军事 쪽에서 지도를 빼고
«나무 표지 여럿»만 남겼다. 나온 것은 해·달·산·나무가 새겨진 **점치는 산가지
세트**였다 — 군사와 아무 상관이 없다. 지도가 그 그림의 유일한 뜻이었다.

**물어야 할 것은 «누가 덜 쓰나»가 아니라 «누가 그것 없이도 제 뜻으로
읽히나»다.** 고친 뒤에는 나팔을 몸통으로 세우고 지도를 배경으로 깔아 둘 다
살렸다. 그리고 바꾼 뒤에는 props를 **한 번 더** 돌린다 — 피하려고 넣은 낱말이
새 겹침을 만든다(24회차에서 `blocks`를 `patch`로 바꿨다가 숨결의 `patch`와
부딪쳤다).

### 글자를 뜻으로 삼는 낱말 — 획보다 임자를 먼저 적는다

括号·字体·栏目·帖子·体裁·转载처럼 **그림에 글자가 없으면 뜻이 서지 않는데
그림에 글자를 못 쓰는** 낱말이 있다. 이미 선례가 있다 — `글자`는 활자 조각에
돋은 획 하나, `글귀`는 종이쪽의 plain marks 한 줄, `새김글`은 돌판에 파인
plain carved lines다. **읽히지 않는 획이 글자 노릇을 한다.**

다만 획만 적으면 모델이 그것을 장식으로 채운다. 宣言을 «벽에 붙인 큰 종이에
plain marks»라고만 적었더니 주황 동그라미와 잎이 놓인 **추상 미술 포스터**가
나왔다. `proclamation sheet` · `brick wall` · `close rows of ink marks` ·
`corner curling`을 박고서야 벽보가 됐다.

**획을 담을 임자를 먼저 적는다.** 종이쪽·신문 면·인쇄틀·비석 탁본처럼 그
획이 어디에 붙어 있는지를 말하면 한 번에 나오고, «plain marks»만 적으면
빈자리가 장식으로 찬다.

### 사람을 가리키는 낱말은 자리·물건·자국으로 그린다

常人·传人·能人·人身·世人처럼 뜻이 곧 사람인 낱말은 그림에 사람을 못 그려
막히는 것처럼 보인다. 2026-09-16에 «人»이 든 낱말 열여덟을 한 회차에 몰아
세워 봤는데 **재생성이 한 장도 없었다.**

- 常人 → 똑같은 사발이 늘어선 시렁
- 传人 → 손잡이가 검게 닳은 끌 옆에 놓인 새 끌
- 人身 → 얼굴 없는 옷본
- 人选 → 검은 조약돌 사발에서 집어내 따로 놓은 흰 돌 하나
- 公约 → 붉은 손도장이 줄줄이 찍힌 두루마리

사람이 앉던 자리, 쓰던 물건, 남긴 자국이면 된다. 덧붙여 **같은 글자를 쓰는
낱말을 한 회차에 몰아도 소품은 안 겹친다**(그 회차 배치 안 겹침 0). 소품은
글자를 따라가지 않고 뜻을 따라간다.

**얼굴이 뜻인 낱말도 같은 길로 간다. 피해 가는 것이 아니라 더 잘 그려진다.**
面红耳赤·目瞪口呆·热泪盈眶·一举一动 넷을 그렇게 세운 회차는 재생성이 0이었다 —
달아오름은 화로에 꽂힌 부지깽이가, 넋 놓음은 떨어져도 안 깨지고 내용물만
동그랗게 흩어진 사발이, 눈물은 접힌 편지에 젖어 든 자국 둘이, 몸짓 하나하나는
보폭이 똑같은 눈 위 발자국이 맡았다. **얼굴을 그리면 표정 하나로 끝나지만
물건은 «무슨 일이 있었는가»까지 같이 그린다.**

### 그릴 것 자체는 안 그려진다 — 곁에 남는 자국으로 그린다

숨김 · 어긋남 · 같음 · 없음 · 닳음 · 움직임 · 시름은 다 다른 낱말인데 **같은
병을 앓는다. 눈에 보이지 않는 것을 그리라고 적으면 모델은 보이지 않게
그린다.** 2026-09-16에 넷을 한꺼번에 만났고, 그 뒤 두 글자 층을 닫는 동안
같은 병이 여섯 번 더 났다.

| 뜻 | 적은 것 | 나온 것 | 통한 말 |
|---|---|---|---|
| 不正之风 비뚤어진 버릇 | 한쪽 접시 **밑에 숨긴** 납덩이 | 그냥 저울 | 받침점을 치우치게 깎아 **대는 수평인데 한쪽만 수북한** 저울 |
| 不当 온당치 않다 | 위 경첩이 아래보다 손가락 둘 어긋난 문 | 그냥 문 | **문틀보다 넓어 닫히지 않고** 한쪽에 햇빛이 쐐기꼴로 드는 문 |
| 共性 두루 같은 바탕 | 잎 다섯인데 **잎맥이 다 같은** 것 | 색도 모양도 다 다른 잎 다섯 | 색이 다른 돌 다섯을 **모두 잘라** 안쪽 흰 켜가 하나같이 같은 것 |
| 约定俗成 오래 써서 굳어지다 | 길을 낸 적 없는데 가로질러 희게 닳은 자리 | 흰 줄이 간 바위 | 손이 닿는 자리만 거울처럼 닳고 나머지는 흐린 **놋쇠 문판** |

두 글자 층을 닫으며 여섯이 더 쌓였다. 갈래는 늘었는데 처방은 하나였다.

| 뜻 | 적은 것 | 나온 것 | 통한 말 |
|---|---|---|---|
| 漠然 덤덤하다 **(없음)** | 먼지가 켜켜이 앉은 문고리 | 그냥 문고리 | 문고리와 문틀 사이에 **끊기지 않은 거미줄** |
| 同感 같이 느낌 **(같음)** | 한 받침에 소리굽쇠 둘, 앞은 울고 뒤는 멈춤 | 가만한 소리굽쇠 둘 | 받침 위 물그릇 둘, **두 물낯에 같은 물결 고리** |
| 任期 자리에 있는 동안 **(닳음)** | 가운데만 매끈하게 닳은 옷걸이 못 | 그냥 옷걸이 | 양옆 못에 **껍질을 남겨** 가운데만 벗겨진 것 |
| 上行 거슬러 오르는 **(움직임)** | 둑길에서 **느슨한** 밧줄로 끌리는 짐배 | 그냥 강에 뜬 배 | 밧줄을 팽팽히 당기고 **뱃머리에 흰 물살** |
| 裸体 걸치지 않은 몸 **(없음)** | 사람 몸 | (거절) | 관절 목각 인형과 **곁상에 개킨 옷** |
| 忧愁 시름에 잠기다 **(마음)** | 빈 곡식통 | «비었다»까지만 | **안 쓴 잠자리**와 창가 의자에 **깊이 팬 자국** |

- **없음은 없는 것으로 못 그린다.** 없음이 남긴 **있는 것**을 그린다 —
  먼지는 물건 위에 얇게 깔려 사라지지만 거미줄은 남는다.
- **같음은 같은 것을 나란히 놓아서 안 된다.** 나란함은 같음이 아니다. 같은
  원인이 남긴 **같은 자국**(두 물낯의 같은 물결 고리)을 그린다.
- **닳음은 견줄 것이 거칠어야 보인다.** 닳은 쪽을 다듬는 것이 아니라 **안 닳은
  쪽을 거칠게** 한다.
- **움직임은 움직이는 것에 안 그려진다.** 그것이 만드는 **자국**(뱃머리의
  물살)에 그려진다. 느슨한 줄은 끌지 않는다.
- **마음은 마음 쓰는 쪽에 안 그려진다.** 밤새 **안 쓰인 것**과 **너무 오래
  쓰인 것**의 차이에 그려진다.
- **둘을 그리려면 둘 사이의 거리와 다름을 따로 적어야 한다.** 「위아래로 남은
  물때 자국 둘」은 한 겹 얼룩으로 뭉쳤고, 「곧은 가로줄 둘, 한 뼘 간격, 아래는
  젖어 번들거리고 위는 바싹 마른」으로 쪼개자 갈렸다(环比).
- **재는 물건은 곧아야 재는 것으로 읽힌다.** 고른 매듭줄이 똬리를 트니
  염주가 되었고, 두 말뚝 사이에 당겨 펴자 자[尺]가 되었다(年均).

- **숨긴 것은 그릴 수 없다.** 숨김이 남긴 **어긋남**을 그린다. 이 레포에는
  선례가 있었다 — 擅自«무단으로»는 자물쇠를 숨기지 않고 «억지로 열린 걸쇠»를
  그렸다.
- **어긋남은 치수로 적으면 흘러간다.** «손가락 둘 어긋난»은 안 보인다. 그
  어긋남이 **막는 일**로 적어야 보인다 — 문이 안 닫힌다, 저울이 안 기운다.
- **같음은 다름 위에 얹으면 흘러간다.** 모델이 «다섯 가지»만 읽고 «같은 것»을
  버린다. **갈라서 단면을 보이면** 못 흘린다.
- **오래 쓴 자국은 닳는 물건을 바꿔 가며 여러 번 쓸 수 있다.** 不知不觉의 닳은
  문지방, 深入人心의 다져진 길, 约定俗成의 닳은 문판이 한 레포에 같이 산다.
  은유가 겹쳐도 **물건이 갈리면 그림은 안 겹친다.**

#### 자국을 어디에 두느냐 — 넷을 잇달아 틀리고 알았다

자국으로 그린다는 것까지는 맞게 적어 놓고도 **자국을 둘 자리**를 네 번
잇달아 틀렸다. TORFL 네 회차에서 회차마다 꼭 한 장씩 다시 그렸는데
네 번 다 이 자리였다.

| 뜻 | 적은 것 | 나온 것 | 통한 말 |
|---|---|---|---|
| 서로 알게 되다 | 쟁반에 나란히 놓인 **명함 둘** | 액자 그림 | 한 못에 걸린 **서로 다른 모자 둘**, 챙을 맞댐 |
| 마음을 졸이다 | 서성여 한 줄만 닳은 **마룻널** | 그냥 마루 | 못에 건 **염주 끈**, 엄지 닿는 한 자리만 희다 |
| 흠을 잡다 | **둥근 통**에 두른 분필 고리 셋 | 그냥 통 | **평평한 접시** 유약 위의 분필 동그라미 셋 |
| 염치가 없다 | 접시 밑에 **몰래 괸 납** | 그냥 저울 | 한쪽만 수북한데 **저울대가 반듯한** 저울 |

세 줄로 줄면 이렇다.

- **자국은 작은 물건 위에 있어야 자국으로 읽힌다.** 넓은 바닥에 난 자국은
  바닥에 먹힌다 — 마룻널의 닳은 줄은 마루가 되고, 염주 끈의 닳은 자리는
  자국이 된다.
- **자국은 평평한 데 있어야 자국으로 읽힌다.** 둥근 배에 그은 선은 그 물건의
  테와 섞여 사라진다.
- **눕힌 종이는 종이가 아니라 판이 된다.** 명함·쪽지처럼 얇은 것을 눕혀
  놓으면 두께가 사라져 액자나 상판으로 읽힌다. 세우거나 걸어야 한다.

**넷째 줄은 새것이 아니다.** 「몰래 괸 납」은 위의 «숨긴 것은 그릴 수 없다»가
이미 不正之风에서 겪고 처방까지 적어 둔 자리인데 그대로 다시 냈다. 적어 둔
함정에 다시 빠진 것이 이번이 **세 번째**다(本意의 화분, 体面의 문간, 이번).
**목록을 읽어 두는 것과 쓸 때 떠오르는 것은 다른 일이다** — 일곱 줄 점검표를
앞에 세운 까닭이 이것이고, 그 5번이 바로 이 자리를 짚는다. 안 돌리면 소용이 없다.

### 솜씨를 연장으로 그릴 때는 그 연장의 쓰임을 먼저 본다

能人«재주 있는 사람»을 «열린 자물쇠와 따개 셋»으로 적었더니 모델이 그림을
아예 내놓지 않았다. 도둑 연장으로 읽힌 것이다. 못 하나 없이 맞물린 짜임으로
바꾸니 한 번에 나왔다.

叫板도 같은 자리였다 — 싸움닭 둘을 마주 세웠더니 두 번 다 빈손이었고,
나무 박자판으로 바꿔서야 나왔다. **거절은 FAIL로 찍히지 다시 뽑는다고
풀리지 않는다.** 연장이나 자세가 다른 일에도 쓰이는 것이면 그 다른 일을
먼저 떠올려 본다.

### 막히는 낱말은 그릴 것이 아니라 **그 자리에 선 것**을 바꿔 푼다

HSK 7-9급에는 사람 몸 · 총 · 약을 곧바로 부르는 낱말이 있다. 그대로 적으면
모델이 아무것도 내놓지 않는다. 다섯을 차례로 만났는데 **다섯 다 그릴 것은
그대로 두고 그 자리에 선 것만 갈아 한 번에 나왔다.**

| 낱말 | 그대로 적으면 | 세운 것 |
|---|---|---|
| 裸体 걸치지 않은 몸 | 사람 몸 | 관절 **목각 인형**과 곁상에 개킨 옷 |
| 枪毙 총으로 쳐 죽이다 | 처형 | **짚 허수아비**에 뚫린 구멍 하나와 새어 나온 짚 |
| 吸毒 독한 것에 빠져 빨다 | 빠는 짓 | 엎어진 등잔에 걸친 대통, 그을린 통과 **흩어진 재** |
| 性感 몸매가 끌리는 | 사람 몸 | **철사 옷틀**에 걸린 비단옷, 조인 허리와 흐르는 자락 |
| 五官 얼굴의 다섯 자리 | 얼굴 | 아직 안 판 **타원 나무토막**에 얕은 자리 다섯 |

세 갈래뿐이다.

- **사람을 세워야 하는 낱말** — 사람 대신 **인형 · 옷틀 · 나무토막**을 세운다.
  사람이 하던 일을 그 물건이 그대로 한다.
- **짓을 그려야 하는 낱말** — 짓이 아니라 **그 짓이 남긴 것**을 그린다
  (뚫린 구멍, 흩어진 재). 위의 「곁에 남는 자국」 절과 같은 처방이다.
- **걸치지 않음 같은 «없음»** — 몸이 아니라 **벗어 둔 옷**이 나른다.

**다시 뽑아서는 안 풀린다.** 能人 · 叫板에서 이미 보았고 여기서도 같다.
거절이 났으면 프롬프트의 말을 다듬지 말고 **주인공을 갈아 끼운다.**

---

## 얼굴 감사 — 인물이 든 그림을 한 번에 본다

인물 그림은 시간이 지나면 **얼굴이 슬금슬금 그려진다.** 프롬프트에
`no facial features`가 빠진 채 생성되면 모델이 표정을 넣기 때문이다.
한 장씩 열어 보면 못 잡으므로 대지(contact sheet)로 묶어서 본다.

```bash
# 인물이 든 프롬프트 가운데 no facial features가 빠진 것만 모아 격자로 붙인다
python3 - <<'EOF'
import json, glob, re
from PIL import Image, ImageDraw
slugs = [c['slug'] for f in glob.glob('content/*.json')
         for c in json.load(open(f))['concepts']
         if re.search(r'\bfigure|\bperson|\bpeople|\bworker\b', (c.get('image_prompt') or '').lower())
         and 'no facial features' not in c['image_prompt'].lower()]
CELL, COLS = 250, 5
for n in range(0, len(slugs), 25):
    group = slugs[n:n + 25]
    rows = (len(group) + COLS - 1) // COLS
    sheet = Image.new('RGB', (COLS * CELL, rows * (CELL + 16)), 'white')
    draw = ImageDraw.Draw(sheet)
    for i, slug in enumerate(group):
        im = Image.open(f'public/concepts/{slug}.webp').convert('RGB').resize((CELL - 8, CELL - 8))
        x, y = (i % COLS) * CELL, (i // COLS) * (CELL + 16)
        sheet.paste(im, (x + 4, y + 4))
        draw.text((x + 6, y + CELL - 2), slug, fill='black')
    sheet.save(f'/tmp/faces{n // 25}.png')
EOF
```

**판정 기준은 입과 눈썹이다.**

| 봐서 | 판정 |
|---|---|
| 옆모습에 눈 점 하나 · 감은 눈 선 | 통과. 규칙이 허용하는 "점 두 개까지" 안이다 |
| 뒷모습 · 얼굴이 가려진 자세 | 통과 |
| **입이 그려졌다 · 눈썹으로 표정을 만들었다** | **위반** — 다시 뽑는다 |

이 감사는 이제 `pnpm check`가 대신한다 — 인물이 든 프롬프트에 그 문구가
없으면 경고가 뜬다. 그림을 눈으로 훑어 잡을 일이 아니라 **문구를 쓸 때 잡을
일**이기 때문이다.

고칠 때는 그림만 다시 뽑지 말고 **`image_prompt` 끝에 `, no facial features`를
붙인 뒤** 뽑는다. 프롬프트를 고치지 않으면 다음에 재생성할 때 같은 얼굴이
돌아온다 — 그림은 프롬프트에서 재현되어야 한다.

---

## 검수 체크리스트

업로드 전 확인한다. 하나라도 걸리면 다시 생성한다.

- [ ] 이미지 안에 글자·숫자·로고가 없다 — **프롬프트가 부르지 않았는지 먼저 본다**
      (`pnpm prompt --glyphs`)
- [ ] 배경이 단색 warm off-white다 (테두리·그림자·그라디언트 없음)
- [ ] 피사체가 하나이고 중앙에 있다
- [ ] **80×80으로 축소해도 무엇인지 알아볼 수 있다** — 가장 중요한 항목
- [ ] 같은 덱의 다른 이미지들과 채도·선 두께가 비슷하다
- [ ] 특정 언어권 문화에만 통하는 표현이 아니다 — 모든 언어가 이 그림을 공유한다
- [ ] 동사·장면에서 인물 얼굴이 과도하게 그려지지 않았다
- [ ] 그림자가 발밑 하나뿐이다
- [ ] `pnpm twins`가 짚은 쌍을 봤다 — 남의 개념과 겹치면
      [docs/twins-pending.md](docs/twins-pending.md)에 적어 임자에게 넘긴다

---

### 숫자를 시키면 모델은 틀린 숫자를 그린다

2026-09-21에 열다섯 장을 뽑아 시트를 보다가 `carry-a-digit`이 「7 + 24 = 36」
으로 나왔다. 프롬프트가 `a small numeral written above a column of two added
numbers`였다 — **숫자를 그리라고 시켰으니 그린 것이고, 그 숫자가 맞을 이유는
없다.** 모델이 헛나간 것이 아니라 규칙을 어긴 프롬프트가 틀린 그림을 부른 것이다.

같은 자리에서 `at-a-disadvantage`는 점수판에 숫자를, `be-booked`은 카드를
**빨갛게** 그렸다(옐로카드인데). 셋 다 md5도 `twins`도 `check`도 못 본다 —
시트를 눈으로 볼 때만 걸린다.

**그래서 뽑기 전에 본다.** `pnpm prompt --glyphs`가 «no letters»로 막지 않은
채 글자·숫자를 부르는 프롬프트를 찍는다. 고치는 법은 그리는 것을 바꾸는 것이다.

    a small numeral written above a column of two added numbers
    → a counting frame with one bead lifted from the lower wire to the wire above

    a numbered shirt and shorts laid flat side by side
    → a plain shirt and shorts laid flat side by side, no letters

색이 뜻을 나르는 자리는 **색을 못 박는다** — 「카드를 든다」가 아니라
「노란 카드를 든다」다.

### 같은 빛깔끼리 겹치면 그 물건은 없는 것이 된다

`give-ones-own-name`(제 이름을 대다)에 «one blank card lying face up in the
middle of an empty tray»라고 썼더니 **쟁반만 나왔다.** 카드는 그려졌는데
미색 위의 미색이라 테두리가 안 보인다. 80×80에서는 말할 것도 없다.

바탕이 미색 한 가지이므로 **흰 것·미색 것·연한 것을 겹쳐 놓으면 사라진다.**
받치는 쪽을 어둡게 못 박는다.

    one blank card lying face up in the middle of an empty tray
    → one pale card lying face up in the middle of a dark stained wooden tray

`pnpm twins`도 `md5`도 이걸 못 본다 — 그림은 멀쩡히 다르다. 시트에서 눈으로
볼 때만 「어? 카드가 없네」가 된다.

### 몸통을 먼저 세면 되돌리지 않는다

2026-09-21에 109장을 일곱 회차로 뽑으며 되돌린 장을 셌다.

| 회차 | 되돌림 | 무엇이 걸렸나 |
| ---: | ---: | --- |
| 1 | 5 | 프롬프트가 글자·숫자를 불렀다 (점수판 · `7 + 24 = 36` · 빨간 카드) |
| 2 | 3 | `table`을 밥상으로 · 시계 둘이 겹침 · **내가 만든 쌍** |
| 3 | 0 | 뽑기 전에 몸통을 세고 넷을 갈랐다 |
| 4 | 0 | 빨래표와 시계를 미리 갈랐다 |
| 5 | 1 | `seat`을 밥상 의자로 그렸다 |
| 6 | 0 | 마지막 열아홉 |

**세는 법.** 그리려는 몸통 낱말을 대여섯 개 골라 `pnpm props <낱말…>`로
한 번에 돌린다. 끝줄의 「다른 개념이 제 몸통으로 쓰는 낱말」에 든 것은
비킨다. 열두 낱말을 골랐더니 아홉이 이미 남의 몸통이었던 회차가 있다 —
**빈 낱말을 먼저 스무 개쯤 뽑아 놓고 거기서 고르는 편이 빠르다.**

**거울상은 재 볼 것도 없이 고치고, 짜임만 닮은 것은 뽑아서 잰다.**
`right-turn`을 `left-turn`의 거울상으로 두면 dHash가 거의 확실히 붙으므로
미리 갈랐다(고친 뒤 112). 반면 눈으로 겁냈던 쌍들은 뽑고 재니 다 멀었다.

    the-stands ↔ well-attended    둘 다 붉은 좌석 더미인데  112
    match-fitness ↔ go-via        둘 다 선으로 그린 도표인데 101
    unpaved ↔ training-ground     둘 다 초록 벌판인데       104

**눈이 겁내는 자리와 기계가 걸리는 자리가 다르다.** 미리 고치면 멀쩡한
그림을 망치므로, 확실한 것만 고치고 나머지는 `pnpm twins --pair`로 잰다.

## 스타일을 바꿀 때

STYLE_PROMPT를 고치면 **기존 이미지와 새 이미지가 섞인다**. 같은 덱 안에서 스타일이
섞이면 학습 화면이 어수선해지므로, 스타일 변경은 덱 단위로 전체 재생성하는 것을 원칙으로 한다.

재생성 절차:
1. 대상 개념의 `image_path`를 `null`로 되돌린다
2. `pnpm prompts` — 새 STYLE_PROMPT로 프롬프트가 다시 나온다
3. 생성 → `.images/` → `pnpm images` (같은 경로에 덮어쓴다)
