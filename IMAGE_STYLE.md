...
그대로 다시 냈다. 그래서 여섯 줄로 줄여 앞에 세운다. 한 줄이라도 걸리면
해당 절을 편다.
...
4. **견줌이 뜻 자체인가.** 그렇다면 둘을 한 몸으로 매야 한다(도르래의 두 추).
   뜻을 돕기만 하면 버린다.
5. **숨김·어긋남·같음을 그리려는가.** 숨긴 것은 안 그려지고, 어긋남은 치수로
   적으면 안 보이며, 같음은 다름 위에 얹으면 흘러간다. 셋 다 **그것이 막는
   일**이나 **갈라 보인 속**으로 적는다.
6. **이 소품을 다른 개념이 쓰고 있지 않은가.** `pnpm props --in`을 그림을
   고르기 **전에** 돌린다.
...
### 숨김 · 어긋남 · 같음은 그 자리에서 안 그려진다

셋은 다른 낱말인데 같은 병을 앓는다. **눈에 보이지 않는 것을 그리라고 적으면
모델은 보이지 않게 그린다.** 2026-09-16에 넷을 한꺼번에 만났다.
