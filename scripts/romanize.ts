...
/**
 * 조사를 떼되 **아는 낱말은 통째로 둔다.**
 *
 * `known`은 콘텐츠에 있는 모든 일본어 읽기다. 그래서 표제어가 하나 늘면 남의
 * 예문에서도 그 자리가 한 덩어리로 읽힌다 — `いっしょに`를 표제어로 넣은 뒤로
 * 남의 예문 `いっしょに はたらいて`가 `issho ni`에서 `isshoni`로 바뀐다.
 *
 * **이건 오염이 아니라 이 함수가 하는 일이다.** 배치를 넣고 나서 `git diff`에
 * 남의 파일이 뜨는 것을 보고 «내 표제어가 남의 로마자를 망쳤다»고 읽기 쉬운데,
 * 대개는 반대다 — `いつでも`가 `itsude mo`로 잘못 읽히던 것이 표제어가 생기면서
 * `itsudemo`로 바로잡힌다. 2026-09-19에 그 자리를 아홉 번 오진해 멀쩡한 표제어를
 * `きぬの`→`きぬおりの`처럼 어색하게 바꿨다가 되돌렸다.
 *
 * **글자 수는 기준이 아니다.** 여섯 자인 `にちようびの`도 붙고 석 자인 `すぐに`도
 * 붙는다. 진짜 탈은 **읽기가 같은데 낱말이 다른 자리**다 — `いえで`(집에서)를
 * 표제어로 넣으면 家出를 적은 남의 예문까지 한 덩어리가 된다. 그때만 표제어를
 * 바꾼다. 가르는 법은 diff의 옛 로마자와 새 로마자를 견주는 것이다. 새 쪽이
 * 그 예문에서도 한 낱말로 읽히면 둔다.
 */
