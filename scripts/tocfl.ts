...
const SEGMENT: Record<string, string> = {
  ...
  /*
   * 5차. 예문 번체가 3문장 남았을 때 나온 둘. 둘 다 `线`이 든 조각이다.
   *
   *   电线  电/電이 갈렸다. 電은 언제나 電이다
   *   折线  打折·折斷과 같은 折다. 摺은 접다 쪽이라 「折線」이 맞다
   *
   * `线`은 콘텐츠가 綫 서른다섯 · 線 열다섯으로 綫이 많다. 자료가 綫만
   * 후보로 내므로 綫을 따른다.
   */
  电线: '電綫', 折线: '折綫',
}
 ...
 *
 * 2026-09-26에 셋을 더 넣었다. 八千詞表·國教院 표로 확인했다.
 *
 *   游 → 遊  노닐다·여행이다(遊戲 · 旅遊 · 導遊 · 遊客 · 遊行 · 遊覽 · 遊艇). 헤엄은
 *           游다(游泳 · 魚在河裡游). 물줄기(上游 · 下游)와 游標(버니어)도 그대로
 *   咨 → 諮  咨詢은 대만에서 諮詢이다(國教院 諮詢 · 諮商)
 *   里 → 裡  「안」이다(村裡 · 海裡 · 市場裡 · 山裡). 公里 · 萬里 · 里程 · 鄉里는 그대로.
 *           裡는 대개 변환이 맞게 내지만, CC-CEDICT가 村里(마을)·海里(해리)를
 *           낱말로 싣는 바람에 이 넷이 里로 남았다
 *
 * 布/佈 · 占/佔은 **그대로 둔다.** 두 표가 다 둘을 함께 싣고(公布/公佈 · 占/佔)
 * 布를 앞에 적는다. 교육부 표준도 布·占이다.
 */
const twGlyph = (text: string) =>
  text
    .replace(/[綫録閑污檐泄卧涌凈棱]/g, (ch) => TW_GLYPH[ch])
    .replace(/游(?=[戲客行艇船覽玩歷])|(?<=[旅導桌日郊周漫出])游/g, '遊')
    .replace(/咨詢/g, '諮詢')
    .replace(/(?<=[村海場山])里/g, '裡')
