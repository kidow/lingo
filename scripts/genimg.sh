...
#
# **빠르게 하려면 PAR을 올리지 말고 프로세스를 나눈다.** 스물두 장을
# `pnpm genimg <앞 열한 개>`와 `pnpm genimg <뒤 열한 개>` 두 번으로 동시에
# 돌렸더니 11분에 끝났고 사고가 없었다(PAR=1 한 프로세스면 22분, PAR=3 한
# 프로세스는 같은 회차에서 중복 하나와 누락 하나가 났다). 프로세스가 다르면
# 임시 디렉터리도 슬러그 목록도 겹치지 않아 사고의 여지가 구조적으로 없다.
# 두 프로세스가 각각 끝 검사를 돌리므로 검사도 그대로 남는다.
#
...
# 같은 장이 두 slug에 들어간 자리. 디렉터리를 나눠도 동시 실행에서 겪었다
files=(); for slug in "$@"; do [ -f "$REPO/.images/$slug.png" ] && files+=("$REPO/.images/$slug.png"); done
if [ ${#files[@]} -gt 1 ]; then
