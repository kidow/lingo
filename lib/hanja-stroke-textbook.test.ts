...
test('교과서 발견 목록과 완료된 검토 등록부를 분리한다', () => {
  ...
  for (const record of TEXTBOOK_REVIEW_REGISTRY.records.filter((entry) => entry.status !== 'matched')) {
    ...
    const independent = [...HANJA_DOCUMENT_STROKES, ...HANJA_NUMBERED_STROKES,
      ...HANJA_DICTIONARY_STROKES, ...HANJA_DICTIONARY_JA_STROKES, ...HANJA_DICTIONARY_G2_STROKES]
  ...
  assert.equal(HANJA_STROKES.length, 503 + completed.length + HANJA_DOCUMENT_STROKES.length + HANJA_NUMBERED_STROKES.length + HANJA_DICTIONARY_STROKES.length + HANJA_DICTIONARY_JA_STROKES.length + HANJA_DICTIONARY_G2_STROKES.length)
})
