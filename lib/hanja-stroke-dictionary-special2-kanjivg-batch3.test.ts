...
import reviewed from '../public/hanja-strokes/dictionary-reviewed-special2-kanjivg-batch3.json' with { type: 'json' }
import { SPECIAL2_KANJIVG_BATCH3_PINS, loadSpecial2KanjiVGBatch3Strokes } from './hanja-stroke-dictionary-special2-kanjivg-batch3.ts'
...
test('鏃·驃·陂·鰕의 검토된 68획과 국내 필순 순열을 재현한다', () => {
  const reproduced = JSON.parse(execFileSync(process.execPath,
    [fileURLToPath(new URL('../docs/hanja-goal-special2-kanjivg-batch3-2026-09-21/build.mjs', import.meta.url))], { encoding: 'utf8' }))
  assert.deepEqual(reviewed, reproduced)
  const normalized = JSON.parse(readFileSync(new URL('../docs/hanja-goal-special2-kanjivg-batch3-2026-09-21/normalized.json', import.meta.url), 'utf8'))
  assert.deepEqual(reviewed.map(({ glyph, sourceStrokeIndices, paths }) => ({ glyph, sourceStrokeIndices, paths })), normalized.entries)
  assert.equal(reviewed.reduce((sum, e) => sum + e.paths.length, 0), 68)
  const findings = JSON.parse(readFileSync(new URL('../docs/hanja-goal-special2-kanjivg-batch3-2026-09-21/findings.json', import.meta.url), 'utf8'))
  for (const pin of SPECIAL2_KANJIVG_BATCH3_PINS) {
  ...
  assert.equal(reviewed[0].geometrySource, '65d5702c3024921929aad1a7635a4245e5cb74fbec030f16fe081b0841439830')
})
...
test('순서 변경과 다른 변형·근거·라이선스 교체를 거부한다', () => {
  ...
  assert.throws(() => loadSpecial2KanjiVGBatch3Strokes(order))
  const variant = structuredClone(reviewed)
  variant[0].geometrySource = '1020e76858a48a5ac428bef5a2b95b34072085e663b2fa5dd8a94384defe8ff2'
  assert.throws(() => loadSpecial2KanjiVGBatch3Strokes(variant))
  const source = structuredClone(reviewed)
  source[0].sourceReference.dictionarySvgUrl = 'https://example.org/unreviewed'
  assert.throws(() => loadSpecial2KanjiVGBatch3Strokes(source))
  const license = structuredClone(reviewed)
  license[0].geometryLicense.spdx = 'unknown'
  assert.throws(() => loadSpecial2KanjiVGBatch3Strokes(license))
  assert.throws(() => loadSpecial2KanjiVGBatch3Strokes([...reviewed, reviewed[0]]))
})
