...
import { loadGlyphWikiBatch36Strokes } from './hanja-stroke-dictionary-glyphwiki-batch36.ts'

test('萃12획을 고정된 전체 원본의 7개 기록에서 재현한다', () => {
  const script = fileURLToPath(new URL('../docs/hanja-goal-glyphwiki-batch36-2026-09-22/build.mjs',import.meta.url))
  assert.deepEqual(JSON.parse(execFileSync(process.execPath,[script],{encoding:'utf8'})),reviewed)
  const data=hanjaStrokeData({glyph:'萃',strokes:12})!
  assert.ok(data)
  assert.equal(HANJA_STROKES.filter(e=>e.glyph==='萃').length,1)
  ...
  assert.equal(hanjaStrokeData({glyph:'萃',strokes:13}),null)
})
test('萃의 경로·순열·선폭·출처·허가 변경을 거부한다', () => {
  for(const modify of [
  ...
  ]){const changed=structuredClone(reviewed);modify(changed[0]);assert.throws(()=>loadGlyphWikiBatch36Strokes(changed))}
  assert.throws(()=>loadGlyphWikiBatch36Strokes([...reviewed,...reviewed]))
})
test('독립 곡선·횡획 분리와 선폭 검증을 실행한다', () => {
  const script=fileURLToPath(new URL('../docs/hanja-goal-glyphwiki-batch36-2026-09-22/convert.test.mjs',import.meta.url))
