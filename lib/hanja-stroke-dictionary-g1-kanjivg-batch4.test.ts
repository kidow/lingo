...
import { G1_KANJIVG_BATCH4_PINS, loadG1KanjiVGBatch4Strokes } from './hanja-stroke-dictionary-g1-kanjivg-batch4.ts'
...
test('饌·鍼의 검토된 순열로 38획을 재현하고 두 보류 글자는 등록하지 않는다', () => {
  const reproduced = JSON.parse(execFileSync(process.execPath,
    [fileURLToPath(new URL('../docs/hanja-goal-g1-batch4-2026-09-21/build.mjs', import.meta.url))], { encoding: 'utf8' }))
  assert.deepEqual(reviewed, reproduced)
  const normalized = JSON.parse(readFileSync(new URL('../docs/hanja-goal-g1-batch4-2026-09-21/normalized.json', import.meta.url), 'utf8'))
  assert.deepEqual(reviewed.map(({ glyph, sourceStrokeIndices, paths }) => ({ glyph, sourceStrokeIndices, paths })), normalized.entries)
  assert.equal(reviewed.reduce((sum, e) => sum + e.paths.length, 0), 38)
  const findings = JSON.parse(readFileSync(new URL('../docs/hanja-goal-g1-batch4-2026-09-21/findings.json', import.meta.url), 'utf8'))
  for (const pin of G1_KANJIVG_BATCH4_PINS) {
  ...
  for (const glyph of ['朕', '讒']) assert.equal(hanjaStrokeData(catalog.find(c => c.glyph === glyph)!), null)
})

test('원본 순서 복구와 근거·라이선스 교체는 공개 번들 검사를 통과하지 못한다', () => {
  const order = structuredClone(reviewed)
  order[0].sourceStrokeIndices = Array.from({ length: 21 }, (_, i) => i + 1)
  assert.throws(() => loadG1KanjiVGBatch4Strokes(order))
  const source = structuredClone(reviewed)
  source[0].sourceReference.dictionarySvgUrl = 'https://example.org/unreviewed'
  assert.throws(() => loadG1KanjiVGBatch4Strokes(source))
  const license = structuredClone(reviewed)
  license[0].geometryLicense.spdx = 'unknown'
  assert.throws(() => loadG1KanjiVGBatch4Strokes(license))
  assert.throws(() => loadG1KanjiVGBatch4Strokes([...reviewed, reviewed[0]]))
})
