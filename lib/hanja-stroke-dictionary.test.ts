...
const candidates = [...originalSources.find(s => s.name === 'MM')!.entries, ...fullOriginals.entries, ...bunJaOriginals.entries, ...walkFourOriginals.entries, ...pyeOriginals.entries, ...rabbitOriginals.entries, ...g4Originals.sources.find(s => s.name === 'Make Me a Hanzi')!.entries].map(e => ({
  character: e.glyph, medians: e.medians, strokes: normalizeMedians(e.medians),
}))
const clone = <T>(value: T): T => structuredClone(value)

test('dictionary crosscheck publishes fourteen reviewed MM candidates including 冊 and 灰', () => {
  ...
  assert.deepEqual(HANJA_DICTIONARY_STROKES.map(e => e.glyph), ['訣', '紋', '森', '楓', '奔', '慈', '蓮', '追', '透', '還', '弊', '兔', '冊', '灰'])
  assert.equal(HANJA_DICTIONARY_STROKES.reduce((n, e) => n + e.paths.length, 0), 153)
...
test('audit reports the dictionary crosscheck separately and reconstructs both corrected and unchanged geometry', () => {
  ...
  assert.equal(audit.verificationSources.dictionary, 14)
  assert.equal(audit.verificationSources.eomunhoe, 0)
  assert.equal(audit.playback['dictionary-crosschecked'], 14)
